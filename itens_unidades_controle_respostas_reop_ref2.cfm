...
<cfquery name="rsPermissaoOrgaoSubordinador" datasource="#dsn_inspecao#">
...
<!---   <cfdump var="#rsPermissaoOrgaoSubordinador#"> --->

<cfquery name="qReop" datasource="#dsn_inspecao#">
   SELECT DISTINCT Rep_Codigo, Rep_Sigla, Rep_Nome
   FROM Reops WHERE Rep_Status = 'A' AND Rep_Codigo = '#qAcesso.Usu_Lotacao#'
   ORDER BY Rep_Sigla
</cfquery>
<!--
<cfquery name="qArea" datasource="#dsn_inspecao#">
	SELECT Ars_Codigo, Ars_Sigla, Ars_Descricao From Areas WHERE Ars_Status = 'A' AND Ars_Codigo = '#rsPermissaoOrgaoSubordinador.Usu_Lotacao#'
</cfquery>
...
<form   method="get" name="form" target="_blank" action="itens_unidades_controle_respostas_reop.cfm" id="form">
      ...
      <tr>
		  <cfif ckTipo eq 1>
			   <td colspan="5" align="center" class="titulo1"><strong><cfoutput>Pontos Pendentes - #qReop.Rep_Nome#</cfoutput></strong></td>
		  </cfif>
		  <cfif ckTipo eq 2>
			   <td colspan="5" align="center" class="titulo1"><strong><cfoutput>Pontos Finalizados - #qReop.Rep_Nome#</cfoutput></strong></td>
		  </cfif>
		...
		<input name="reop" type="hidden" id="reop" value="<cfoutput>#rsPermissaoOrgaoSubordinador.Usu_Lotacao#</cfoutput>">
	 	    <tr>
	 	      <td colspan="5">&nbsp;</td>
           </tr>
		    <tr>
		    <td align="right"><span class="exibir"><strong><label title="label">Selecione seu órgão ou todos subordinados:</label></strong></span></td>
		    <td colspan="4" align="left"><select name="cbReop" class="form">
			<cfoutput>
				
			  <option value=#qReop.Rep_Codigo# selected>#qReop.Rep_Sigla#</option>	
			</cfoutput>	
		      <option value="todos">TODAS AS UNIDADES SUBORDINADAS</option>
            </select></td>
		    </tr>
		     <tr>
	 	      <td colspan="5">&nbsp;</td>
