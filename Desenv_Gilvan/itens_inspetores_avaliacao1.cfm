...
<!--- 	</cfif>
    ...
    <cfif Form.acao is "Salvar" >  --->
		 ...
		 <cfif qSemAvaliacoes.recordcount eq 0>
          ...
          <!---Ao salvar, se o inspetor Nao inseriru uma resposta ao gestor, o form de resposta é aberto          --->
	      <cfquery name="qRecomendacao" datasource="#dsn_inspecao#">
      ...
      <!--- 	Fim da rotina que abre o form de inclusão de resposta ao gestor	 --->
		...
		<cfquery datasource="#dsn_inspecao#" name="rsVerificaAvaliador">
			SELECT RIP_MatricAvaliador, RIP_Resposta, RIP_Falta, RIP_REINCINSPECAO FROM Resultado_Inspecao 
...
<!--- inicio classificacao do ponto --->
					...
					<cfset ClasItem_Ponto = rsitem2.Itn_Classificacao>
					<cfset auxfrmfalta = Replace(rsVerificaAvaliador.RIP_Falta,',','.','All')>
				<!--- 	<cfset ItnPTCSeq = rsitem2.Itn_PTC_Seq> --->
...
<cfelse>
	...
	<cfparam name="URL.frminspreincidente" default=""> 
...
<cfif grpacesso eq 'INSPETORES'>
	...
	<cfelse>
		<cfquery name="rsSeguir" datasource="#dsn_inspecao#">
			SELECT Pos_Inspecao, Pos_Unidade, Pos_NumGrupo, Pos_NumItem, Pos_Situacao_Resp, STO_Descricao
			FROM ParecerUnidade INNER JOIN Situacao_Ponto ON Pos_Situacao_Resp = STO_Codigo
			WHERE Pos_Unidade ='#URL.Unid#' AND Pos_NumGrupo = #URL.Ngrup# AND Pos_NumItem = #URL.Nitem# and Pos_Situacao_Resp in(3,25,26,27,29)
...
//-----------------------------------
		if (a == 'N'){
			...
			if (frm.frmrestringirsn.value == 'S') { 
		        var auxmelhoria = frm.frmanexoscomentario.value;
			    var auxrecomendacao = frm.frmanexosrecomendacao.value;
			}	
						
			if (frm.acao.value == 'anexar' || frm.acao.value == 'excluir_anexo') { 
...
//	alert('linha 680 avaliacao: ' + a + ' ripresposta: ' + ripresposta);
//-------------------------------------------------------	
		window.reincidA.style.visibility = 'hidden';
		window.reincidB.style.visibility = 'hidden';	
...
//alert(a + ' ' + b + '  ' + c);
	...
	if (a != 'N' && a != 'A')
	{
	document.getElementById("frmfalta").value = '0,00';
	document.getElementById("frmsobra").value = '0,00';
	document.getElementById("frmemrisco").value = '0,00';
	
	document.getElementById("frmfalta").readOnly = true;
	document.getElementById("frmsobra").readOnly = true;
	document.getElementById("frmemrisco").readOnly = true;
	} 
	
//if (a == 'N' && b == 'S') {
if (a == 'N' && b == 'S') {
	document.getElementById("frmemrisco").readOnly = true;
	document.getElementById("frmfalta").readOnly = false;
	document.getElementById("frmsobra").readOnly = false;
	if (c == 'S') {document.getElementById("frmemrisco").readOnly = false;}
	}	
}
//--------------------------
...
/*  
if (a == 'N' && d == 'S' && (c == 'SOLUCIONADO' || c == 'ENCERRADO')) {
  var texto = 'Sr(a). Inspetor(a),\n\nCom base nas informações prestadas, o item avaliado foi \n\nconsiderado reincidente ( Relatório ' + g + ' ) no que \n\nrestringe essa avaliação à condição de "NÃO CONFORME" apenas.';
  }
 if (a == 'N' && d == 'N' && (c == 'SOLUCIONADO' || c == 'ENCERRADO')) {
  var texto = 'Sr(a). Inspetor(a),\n\nCom base nas informações prestadas, o item avaliado foi \n\nconsiderado reincidente ( Relatório ' + g + ' ), porém, \n\nesta avaliação estará aberta a outros tipos de avaliaçãos.';
  }  
  */
...
//===================================================================================
		...
		if (caracvlr == 'QUANTIFICADO' && auxavaliacao == 'N' && (falta == '' || falta == '0,00') && (sobra == '' || sobra == '0,00') && (emrisco == '' || emrisco == '0,00'))
		{
		//		alert('Para o Tipo "QUANTIFICADO" informe os campos Falta e ou Sobra e ou Em Risco!');
		//		document.getElementById("aguarde").style.visibility = "hidden";
		//		return false;
			if(confirm('Gestor(a), O ponto avaliado "TEM IMPACTO FINANCEIRO DIRETO".\n\nOs campos de Falta e Sobra e Em Risco estão zerados\n\nConfirma os valores informados?'))
			{ return true;	} 
			 else
			{
			document.getElementById("frmfalta").focus();
			document.getElementById("aguarde").style.visibility = "hidden";
				return false;
			}
		}				
...
//-------------------------------------------------------------		
	...
	reg_melhoria();	
<!--- 	AvaliacaoOnChangeModelos(document.form1.avalItem.value);
	AvaliacaoOnChange(document.form1.avalItem.value); --->
	}
...
<!--- 				<cfset restringirSN = 'N'> --->
		...
		<script>avisoreincidencia('<cfoutput>#auxvlr#</cfoutput>','<cfoutput>#frminspreincidente#</cfoutput>','<cfoutput>#trim(frmdescantes)#</cfoutput>','<cfoutput>#restringirSN#</cfoutput>', '<cfoutput>#url.Ngrup#</cfoutput>','<cfoutput>#Nitem#</cfoutput>')
		</script>
</cfif>	
...
<cfif rsItem.RIP_REINCINSPECAO neq 0>
	...
	<body  onLoad="valores(document.form1.avalItem.value,document.form1.caracvlrhabilSN.value,document.form1.auxriscoSN.value);
...
<!---  	hanci(); AvaliacaoOnChange('<cfoutput>#rsItem.RIP_Resposta#</cfoutput>'); if(document.form1.frmrestringirsn.value == 'S'){AvaliacaoOnChangeModelos(document.form1.frmauxvlr)};if(document.form1.houveProcSN.value != 'S') {exibe(document.form1.frmResp.value)} else {exibe(24)}; exibe(document.form1.frmResp.value); controleNCI();"> --->  
...
<table width="85%"  align="center" bordercolor="f7f7f7">
  <tr>
    <td height="20" colspan="9"><div align="center"><strong class="titulo2"><cfoutput>#rsItem.Dir_Descricao#</cfoutput></strong></div></td>
  </tr>
  <tr>
    <td height="20" colspan="9">&nbsp;</td>
  </tr>
  <tr>
    <td height="10" colspan="9"><div align="center"><strong class="titulo1">AVALIACAO ITEM</strong></div></td>
  </tr>
...
<!--- variaveis de formulario --->
	...
	<input name="retornosn" id="retornosn" type="hidden"  value="<cfoutput>#URL.retornosn#</cfoutput>">
	<input name="frmsituantes" id="frmsituantes" type="hidden"  value="<cfoutput>#URL.frmsituantes#</cfoutput>">
	<input name="frmdescantes" id="frmdescantes" type="hidden"  value="<cfoutput>#URL.frmdescantes#</cfoutput>">
...
<cfif isDefined("Form.acao") And (Form.acao is 'Anexar' Or Form.acao is 'excluir_anexo')>
 <cfset auxvlr = 'N'>
  <cfset auxopcao = 'N'>
<cfelse>
  <cfset auxopcao = rsItem.RIP_Resposta>
</cfif> 

<cfif isDefined('retornosn')>
   <cfset auxopcao = 'N'>
</cfif>
<cfif auxvlr is ''>
 <cfset auxopcao = rsItem.RIP_Resposta>
	...
	<tr class="exibir">
		<td bgcolor="f7f7f7" class="exibir" style="background:transparent;font-size:14px"><STRONG>AVALIA&Ccedil;&Atilde;O:</STRONG></td>
		<td colSpan="3" >
		<!---  --->
		
		<cfif restringirSN eq 'S'>
			<select name="avalItem" id="avalItem" class="form" style="background:white;font-size:14px">
				<option value="#auxvlr#">#auxdescvlr#</option>
			</select>
		<cfelse>
		    
<select name="avalItem" id="avalItem" class="form" onChange="valores(this.value,document.form1.caracvlrhabilSN.value,document.form1.auxriscoSN.value);reincidencia(this.value);rsseguir(this.value,'#qtdfrmrsSeguir#','#Ngrup#','#Nitem#');if (document.form1.acao.value != 'reincidencia') {AvaliacaoOnChangeModelos(this.value)};" style="background:white;font-size:14px"> 			
				<cfif #rsItem.RIP_NumGrupo# eq 500 and #rsItem.RIP_NumItem# eq 1>	
				    <option <cfif '#auxopcao#' is "A">selected</cfif>  value="A">---</option>			
					<option <cfif '#auxopcao#' is "N">selected</cfif>  value="N">NÃO CONFORME</option>
					<option <cfif '#auxopcao#' is "V">selected</cfif>  value="V">NÃO VERIFICADO</option>
				<cfelse>
					<option <cfif '#auxopcao#' is "A">selected</cfif>  value="A">---</option>				
					<option <cfif '#auxopcao#' is "C">selected</cfif>  value="C">CONFORME</option>
					<option <cfif '#auxopcao#' is "N">selected</cfif>  value="N">NÃO CONFORME</option>
					<option <cfif '#auxopcao#' is "V">selected</cfif>  value="V">NÃO VERIFICADO</option>
					<option <cfif '#auxopcao#' is "E">selected</cfif>  value="E">NÃO EXECUTA</option>
				</cfif>
			</select>	
		</cfif>	
		<!---  --->
	  ...
	  <tr>
	    <td bgcolor="f7f7f7" class="exibir">Unidade</td>
	    <td colspan="7" bgcolor="f7f7f7"><table width="95%">
          <tr>
            <td width="580"><cfoutput><strong class="exibir">#URL.Unid#</strong> - <strong class="exibir">#rsMOd.Und_Descricao#</strong></cfoutput></td>
            <td width="86"><span class="exibir">Respons&aacute;vel:</span></td>
            <td width="334"><cfoutput><strong class="exibir">#qResponsavel.INP_Responsavel#</strong></cfoutput></td>
          </tr>
        </table></td>
      </tr>
	  
 <tr class="exibir">
   <td width="101" bgcolor="f7f7f7" class="exibir">
    ...
    <tr class="exibir">
      <td bgcolor="f7f7f7">N&ordm; Relat&oacute;rio</td>
      <td colspan="8" bgcolor="f7f7f7"><cfoutput><strong class="exibir">#URL.Ninsp#</strong></cfoutput>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;In&iacute;cio Inspe&ccedil;&atilde;o &nbsp;<strong class="exibir"><cfoutput>#DateFormat(rsItem.INP_DtInicInspecao,"dd/mm/yyyy")#</cfoutput></strong></td>
      </tr>
    <tr class="exibir">
      <td bgcolor="f7f7f7">Grupo</td>
      <td colspan="8" bgcolor="f7f7f7"><cfoutput><strong class="exibir">#URL.Ngrup#</strong></cfoutput>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<cfoutput><strong class="exibir">#URL.DGrup#</strong></cfoutput></td>
    </tr>

    <tr class="exibir">
      <td bgcolor="f7f7f7">Item</td>
      <td colspan="8" bgcolor="f7f7f7"><cfoutput><strong class="exibir">#URL.Nitem#</strong></cfoutput>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<cfoutput><strong class="exibir">#rsItem.Itn_Descricao#</strong></cfoutput><cfoutput></cfoutput>        <div align="right"></div>        <cfoutput></cfoutput></td>
	  </tr>
<cfoutput>
  ...
  <cfif rsItem.Pos_ClassificacaoPonto neq ''>
 	...
 	<tr class="exibir">
	  <td bgcolor="f7f7f7">Relevância Item </td>
	...
	<cfelseif isDefined("frminspreincidente") and len(trim(frminspreincidente)) gt 0 and auxdescvlr neq 'CONFORME'>
		 ...
		 <tr class="red_titulo">
	  <td bgcolor="f7f7f7"><div id="reincidA">Reincid&ecirc;ncia</div></td>  
	   
	      <input type="hidden" name="db_reincSN" id="db_reincSN" value="#reincSN#">
		   
	<td colspan="4" width="634"  bgcolor="f7f7f7"><div id="reincidB">
N&ordm; Relat&oacute;rio:
			...
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;N&ordm; Item:
			<input name="frmreincItem" type="text" class="form" id="frmreincItem" size="7" maxlength="4" value="#db_reincItem#" style="background:white" readonly=""></div></td>
	</tr> 	
	</cfoutput>	    
   <cfoutput>
	<cfset caracvlrhabilSN = 'N'>
	...
	<cfif listFind(#rsItem.Itn_PTC_Seq#,10)>
	   ...
	   <cfif (Ngrup is 230 and Nitem is 1) or (Ngrup is 232 and Nitem is 1)>
	   <cfset auxriscoSN = 'S'>
	   </cfif>
	</cfif>	
...
<!--- composicao item: #rsItem.Itn_PTC_Seq# --->
 <tr class="exibir">
      ...
      <td colspan="4" bgcolor="f7f7f7">
		  <table width="96%" border="0" cellspacing="0" bgcolor="eeeeee" >
		  <cfif caracvlrhabilSN is 'S'>
		  <tr class="form">
					<td width="24%" bgcolor="f7f7f7">Caracteres:&nbsp;
					  <select name="caracvlr" id="caracvlr" class="form">
						<option  value="QUANTIFICADO">QUANTIFICADO</option>
                      </select>                    </td>
<td width="19%">Falta(R$):&nbsp;<input name="frmfalta" type="text" class="form" value="#falta#" size="22" maxlength="17" onFocus="moeda_dig(this.name)" onKeyPress="moeda_dig(this.name)" onKeyUp="moeda_edit(this.name)" onBlur="ajuste_campo(this.name)"></td>
					<td width="22%">Sobra(R$):&nbsp;<input name="frmsobra" type="text" class="form" value="#sobra#" size="22" maxlength="17" onFocus="moeda_dig(this.name)" onKeyPress="moeda_dig(this.name)" onKeyUp="moeda_edit(this.name)" onBlur="ajuste_campo(this.name)"></td>
					<td width="35%" colspan="2">Em Risco(R$):&nbsp;<input name="frmemrisco" type="text" class="form" value="#emrisco#" size="22" maxlength="17" onFocus="moeda_dig(this.name)" onKeyPress="moeda_dig(this.name)" onKeyUp="moeda_edit(this.name)" onBlur="ajuste_campo(this.name)"></td>
				</tr>
		  <cfelse>
		  		<tr class="form">
					<td>Caracteres:&nbsp;
					...
					<td>Falta(R$):&nbsp;<input name="frmfalta" type="text" class="form" value="#falta#" size="22" maxlength="17" readonly=""></td>
					<td>Sobra(R$):&nbsp;<input name="frmsobra" type="text" class="form" value="#sobra#" size="22" maxlength="17" readonly=""></td>
					<td>Em Risco(R$):&nbsp;<input name="frmemrisco" type="text" class="form" value="#emrisco#" size="22" maxlength="17" readonly=""></td>
		  		</tr>
		  </cfif>
		  </table>		  </td>
      </tr> 
...
<tr>
		 ...
		 <td colspan="9" bgcolor="f7f7f7"><textarea name="Melhoria" id="Melhoria" style="background:#fff;display:none!important;" cols="200" rows="25" wrap="VIRTUAL" class="form"><cfoutput>#form.Melhoria#</cfoutput></textarea></td>
</tr>
...
<textarea  hidden name="ripcomentario" id="ripcomentario" style="background:#fff;display:none!important;" cols="200" rows="25" wrap="VIRTUAL" class="form"><cfoutput>#rsItem.RIP_Comentario#</cfoutput></textarea>
<textarea  hidden name="itnprerelato" id="itnprerelato" style="background:#fff;display:none!important;" cols="200" rows="25" wrap="VIRTUAL" class="form"><cfoutput>#rsItem.Itn_PreRelato#<strong>Ref. Normativa: </strong>#Trim(rsItem.Itn_Norma)#<p><strong>Possíveis Consequências da Situação Encontrada:</strong></cfoutput></textarea>
...
<tr>
			...
			<td colspan="9" bgcolor="f7f7f7"><textarea  name="recomendacao" id="recomendacao" style="background:#fff;" cols="200" rows="7" wrap="VIRTUAL" class="form"><cfoutput>#form.recomendacao#</cfoutput></textarea></td>
</tr>
...
<cfif numncisei neq "">
	...
	<div id="divAnexos" name="divAnexos" >
		...
		<tr>
		<td colspan="9" class="exibir" bgcolor="eeeeee"><div align="left"><strong class="exibir">ANEXOS</strong></div></td>
		</tr>
		...
		<cfset existeanexos = 'S'>
			...
			<td width="360" bgcolor="#eeeeee"><cfset arquivo = ListLast(qAnexos.Ane_Caminho,'\')>
					<div align="left">
						&nbsp;
			...
			<td colspan="4" bgcolor="eeeeee"><cfoutput>
				<div align="center">
					<input id="Excluir" name="Excluir" type="submit" class="botao" onClick="document.form1.acao.value='excluir_anexo';document.form1.vCodigo.value=this.codigo;<cfif "#rsItem.RIP_Recomendacao#" is 'S'>document.form1.emReanalise.value='S';</cfif>;document.form1.recomendacao.disabled = false;" value="Excluir" codigo="#qAnexos.Ane_Codigo#">
				  </div>
			</cfoutput></td>
			</tr>
		</cfif>
	...
	<tr>
      <td colspan="9">&nbsp;</td>
	    ...
	    <tr>
      <td colspan="9" align="center"><cfoutput>
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<cfif '#trim(qAcesso.Usu_GrupoAcesso)#' eq "INSPETORES"> 
            <input name="btnsalvar" id="btnsalvar" type="Submit" class="botao" value="Salvar" disabled="disabled" onClick="CKupdate();document.form1.caracvlr.disabled = false;document.form1.acao.value='Salvar';<cfif "#rsItem.RIP_Recomendacao#" is 'S'>document.form1.emReanalise.value='R';</cfif>">
		</cfif>
      </cfoutput></td>
    </tr>
   ...
   <input type="hidden" name="frmexisteanexos" id="frmexisteanexos" value="<cfoutput>#existeanexos#</cfoutput>">
	...
	<cfif restringirSN eq 'S'>
		...
		<script>
			AvaliacaoOnChangeModelos('<cfoutput>#auxvlr#</cfoutput>')
		</script>
	<cfelse>
	  <input type="hidden" name="frmauxvlr" id="frmauxvlr" value="<cfoutput>#trim(rsItem.RIP_Resposta)#</cfoutput>">
	</cfif>
<!--- 	<cfoutput>restringirSN: #restringirSN#</cfoutput><br>
	<cfoutput>auxvlr: #trim(auxvlr)#</cfoutput><br>
	<cfoutput>rsItem.RIP_Resposta: #trim(rsItem.RIP_Resposta)#</cfoutput>	 --->
