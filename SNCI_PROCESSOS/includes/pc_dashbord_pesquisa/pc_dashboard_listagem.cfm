...
$(document).ready(function() {
    ...
    // Função para configurar e retornar a instância do DataTable - movida do arquivo principal para este componente
    function configurarDataTable() {
        ...
        // Se a tabela já foi inicializada, destruir para recriá-la
    ...
    // Função para carregar os dados da tabela - esta é a função chamada do arquivo principal
    window.carregarDadosTabela = function(anoFiltro, mcuFiltro) {
        console.log("Carregando dados para tabela:", anoFiltro, mcuFiltro);
        
        // Configurar DataTable primeiro
        var tabela = configurarDataTable(); // Chama a função local agora
    ...
    setTimeout(function() {
        ...
        const mcuSelecionado = $("input[name='opcaoMcu']:checked").val() || 'Todos';
