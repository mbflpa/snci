...
<cfprocessingdirective pageencoding = "utf-8">	
						...
						<div class="card-body shadow" style="border: 2px solid ##34a2b7">
							...
							<cfelse>
								...
								<div id="divResultadoDGCI" class="col-md-5 col-sm-5 col-12 mx-auto">
									...
									<cfset var criarCardIndicadorDGCI = criarCardIndicador(
										tipoDeCard = 'bg-info',
										siglaIndicador ='DGCI',
											...
											<cfloop index="month" from="1" to="#arguments.mes#">
												...
												<cfloop query="rsOrgaosDGCI">
													...
													<cfelseif metaPRCI neq "" and metaSLNC eq "">
														<cfset metaDGCI = metaPRCI >
													<cfelseif metaPRCI eq "" and metaSLNC neq "">
														<cfset metaDGCI = metaSLNC >
													<cfelse>
														<cfset metaDGCI = 0>
													</cfif>
									...
									<tbody>
										...
										<cfloop query="resultadoDGCIporOrgaoSubordinador" >
											...
											<cfoutput>	
												...
												<cfquery name="metaPRCIporOrgao" datasource="#application.dsn_processos#">
													SELECT  coalesce(AVG(meta),0) AS metaMes from (
														SELECT DISTINCT pc_indOrgao_ano, pc_indMeta_mes, pc_indOrgao_mcuOrgaoSubordinador, pc_indMeta_mcuOrgao, pc_indicadores_meta.pc_indMeta_meta as meta from pc_indicadores_porOrgao
														INNER JOIN pc_indicadores_meta ON pc_indicadores_porOrgao.pc_indOrgao_mcuOrgao = pc_indicadores_meta.pc_indMeta_mcuOrgao 
																and pc_indicadores_porOrgao.pc_indOrgao_numIndicador = pc_indicadores_meta.pc_indMeta_numIndicador and pc_indicadores_porOrgao.pc_indOrgao_ano = pc_indicadores_meta.pc_indMeta_ano and pc_indicadores_porOrgao.pc_indOrgao_mes = pc_indicadores_meta.pc_indMeta_mes
																WHERE pc_indOrgao_mcuOrgaoSubordinador = <cfqueryparam value="#pc_indOrgao_mcuOrgao#" cfsqltype="cf_sql_varchar">
	...
	<cffunction name="cardPRCI_AcompDiario" access="remote" returntype="string" hint="cria o card com as informações dos resultados do PRCI para acompanhamento diário">
		...
		<cfset var cardPRCIdiario = criarCardIndicador(
	...
	<cffunction name="cardSLNC_AcompDiario" access="remote" returntype="string" hint="cria o card com as informações dos resultados do SLNC para acompanhamento diário">
		...
		<cfset var cardSLNCdiario = criarCardIndicador(
			tipoDeCard = 'bg-gradient-warning',
			siglaIndicador ='SLNC',
    ...
    <cffunction name="cardDGCI_AcompDiario" access="remote" returntype="string" hint="cria o card com as informações dos resultados do GDCI para acompanhamento diário">
		...
		<cfset var cardDGCI = criarCardIndicador(
							...
							<div class="table-responsive ">
								<table id="tabPRCIporOrgaoSubordinador" class="table table-bordered table-striped text-nowrap " style="border-left: 1px solid #dee2e6;border-bottom: 1px solid #dee2e6;">
									<cfoutput>
										...
										<tbody>
											<cfoutput>
												<cfloop query="resultado"> <!-- Inicia um loop que itera sobre o conjunto de dados resultado -->
													...
													<cfquery name="rsMetaPRCI" datasource="#application.dsn_processos#">
														SELECT  AVG(meta) AS pc_indMeta_meta from (
															SELECT DISTINCT pc_indMeta_ano
																			,pc_indMeta_mes
																			,pc_indOrgao_mcuOrgaoSubordinador
																			,pc_indMeta_mcuOrgao
																			,pc_indicadores_meta.pc_indMeta_meta as meta 
															...
															WHERE pc_indOrgao_mcuOrgaoSubordinador = <cfqueryparam value="#mcuOrgao#" cfsqltype="cf_sql_varchar">
																AND pc_indMeta_numIndicador = <cfqueryparam value="1" cfsqltype="cf_sql_integer">
										...
										<tfoot >
											<tr>
												<th colspan="7" style="font-weight: normal; font-size: smaller;">
													<li>Meta % = média das metas dos órgãos subordinados e do próprio órgão subordinador (caso tenha sido responsável por alguma orientação).</li>
												</th>
											</tr>
										</tfoot>
									</cfoutput>
	...
	<cffunction name="tabDGCIorgaosSubordinadores_mensal_CI" access="remote" returntype="string" hint="cria tabela resumo com as informações dos resultados do DGCI dos órgãos subordinadores para consulta do Controle Interno">
									...
									<cfoutput>
										...
										<tbody>
											<cfoutput>
												<cfloop query="resultado"> <!-- Inicia um loop que itera sobre o conjunto de dados resultado -->
													<cfquery name="rsMetaPRCI" datasource="#application.dsn_processos#">
														SELECT  ROUND(AVG(meta),1) AS pc_indMeta_meta from (
															SELECT DISTINCT pc_indMeta_ano
																			,pc_indMeta_mes
																			,pc_indOrgao_mcuOrgaoSubordinador
																			,pc_indMeta_mcuOrgao
										...
										<tfoot >
											<tr>
												<th colspan="9" style="font-weight: normal; font-size: smaller;">
													<li>PRCI % = média dos resultados dos órgãos subordinados e do próprio órgão subordinador (caso tenha sido responsável por alguma orientação);</li>
													<li>SLNC % = média dos resultados dos órgãos subordinados e do próprio órgão subordinador (caso tenha sido responsável por alguma orientação).</li>
													<li>META % = média das metas dos órgãos subordinados e do próprio órgão subordinador (caso tenha sido responsável por alguma orientação) = (média meta PRCI * peso PRCI) + (média meta SLNC * peso SLNC)</li>
												</th>
											</tr>
										</tfoot>
									</cfoutput>
		...
		<script language="JavaScript">
			...
			var tituloExcel_DGCIporOrgaoSubordinador ="SNCI_Consulta_DGCI_porOrgaoSubordinador_";
				const tabDGCIporOrgaoSubordinador = $('#tabDGCIporOrgaoSubordinador').DataTable( {
					destroy: true, // Destruir a tabela antes de recriá-la
					stateSave: false,
					deferRender: true, // Aumentar desempenho para tabelas com muitos registros
					scrollX: true, // Permitir rolagem horizontal
	...
	<cffunction name="tabDGCIorgaosResposaveis_mensal_CI" access="remote" returntype="string" hint="cria tabela resumo com as informações dos resultados do DGCI dos órgãos subordinadores para consulta do Controle Interno">
											...
											<cfoutput>
												<cfloop query="resultado"> <!-- Inicia um loop que itera sobre o conjunto de dados resultado -->
													...
													<tr style="font-size:12px;cursor:auto;z-index:2;text-align: center;"  >
														...
														<cfif rsMetaPRCI.pc_indMeta_meta neq ''>
															<cfset metaPRCI = NumberFormat(ROUND(rsMetaPRCI.pc_indMeta_meta*10)/10,0.0)>
														<cfelse>
															<cfset metaPRCI = ''>
														</cfif>

														<cfif rsMetaSLNC.pc_indMeta_meta neq ''>
															<cfset metaSLNC = NumberFormat(ROUND(rsMetaSLNC.pc_indMeta_meta*10)/10,0.0)>
														<cfelse>
															<cfset metaSLNC = ''>
														</cfif>
		...
		<script language="JavaScript">
			...
			var tituloExcel_DGCIporOrgaoResponsavel ="SNCI_Consulta_DGCI_porOrgaoResponsavel_";
				const tabDGCIporOrgaoResponsavel = $('#tabDGCIporOrgaoResponsavel').DataTable( {
					destroy: true, // Destruir a tabela antes de recriá-la
					stateSave: false,
					deferRender: true, // Aumentar desempenho para tabelas com muitos registros
					scrollX: true, // Permitir rolagem horizontal
	...
	<cffunction name="tabDGCImes_A_mes_mensal_CI" access="remote" returntype="string" hint="cria tabela com as informações dos resultados do DGCI mês a mês para consulta do Controle Interno">
		...
		<script language="JavaScript">
			...
			const tabIndicadorDGCI_Mes_a_Mes = $('#tabIndicadorDGCI_Mes_a_Mes').DataTable( {
				destroy: true, // Destruir a tabela antes de recriá-la
				ordering: false, // Desativa a classificação
				lengthChange: false, // Desabilita a opção de seleção da quantidade de páginas
				paging: false, // Remove a paginação
	...
	<cffunction name="cardDGCI_ECT_mes_mensal_CI" access="remote" returntype="string" hint="cria tabela com as informações dos resultados do DGCI mês a mês para consulta do Controle Interno">
		...
		<cfquery name="rsIndicadoresPorOrgao" datasource="#application.dsn_processos#" timeout="120"  >
			...
			WHERE pc_indOrgao_ano = <cfqueryparam value="#ano#" cfsqltype="cf_sql_integer">
			and pc_indOrgao_mes = <cfqueryparam value="#mes#" cfsqltype="cf_sql_integer">
			and pc_indOrgao_numIndicador in(1,2)
			and pc_indOrgao_paraOrgaoSubordinador = 1
		</cfquery>

		<cfquery name="resultadoECTmes" dbtype="query"  >
			SELECT  pc_indOrgao_numIndicador
					,AVG(pc_indOrgao_resultadoMes) AS media_resultadoMes
					,AVG(pc_indOrgao_resultadoAcumulado) AS media_resultadoAcumulado
			FROM rsIndicadoresPorOrgao
			GROUP BY pc_indOrgao_numIndicador
		</cfquery>
		...
		<cfif resultadoECTmes.media_resultadoMes[1] neq '' >
			<cfset mediaPRCI = NumberFormat(ROUND(resultadoECTmes.media_resultadoMes[1]*10)/10,0.0)>
		<cfelse>
			<cfset mediaPRCI = ''>
		</cfif>

		<cfif resultadoECTmes.media_resultadoMes[2] neq '' >
			<cfset mediaSLNC = NumberFormat(ROUND(resultadoECTmes.media_resultadoMes[2]*10)/10,0.0)>
		<cfelse>
			<cfset mediaSLNC = ''>
		</cfif>

		<cfif mediaPRCI neq '' and mediaSLNC neq ''>
			<cfset DGCI = NumberFormat(ROUND((mediaPRCI*rsPRCIpeso.pc_indPeso_peso + mediaSLNC*rsSLNCpeso.pc_indPeso_peso)*10)/10,0.0)>
		<cfelse>
			<cfset DGCI = ''>
		</cfif>
