...
<cfquery name="rsMenuGrupos" datasource="#dsn_processos#">
	SELECT DISTINCT pc_controle_acesso.pc_controle_acesso_grupoMenu FROM pc_controle_acesso
</cfquery>

<cfquery name="rsMenuSubGrupos" datasource="#dsn_processos#">
	SELECT DISTINCT pc_controle_acesso.pc_controle_acesso_subgrupoMenu FROM pc_controle_acesso
</cfquery>
						...
						<div id="cadastroLinks" class="card card-primary collapsed-card" >
							...
							<div class="card-body" style="border: solid 3px #0083ca">
								<div class="row  mb-2" style="font-size:14px">
									...
									<div class="col-sm-6">
										<div class="form-group">
											<label for="controleNomeLink" >Descrição Link que aparecerá no menu:</label>
											<input id="controleNomeLink"  required="true"  name="controleNomeLink" type="text" class="form-control "  inputmode="text" placeholder="Informe o nome do link...">
										</div>
									</div>

									<div class="col-sm-6" style="box-shadow:0!important">
										<div class="form-group " >
											<label for="iconesLink">Ícone do Link:</label>
											<select id="iconesLink" required="false"  class="form-control icons_select2" >
								...
								<div class="row" style="font-size:14px">
									...
									<div class="col-sm-6 " style="box-shadow:0!important">
										<div class="form-group " >
											<label for="iconesGrupo">Ícone do Grupo:</label>
											<select id="iconesGrupo" required="false"  class="form-control icons_select2" >
									...
									<div class="col-sm-6" style="box-shadow:0!important">
										<div class="form-group " >
											<label for="iconesSubgrupo">Ícone do Subgrupo:</label>
											<select id="iconesSubgrupo" required="false"  class="form-control icons_select2" >
												<option selected="" value=""></option>
												<cfloop list="#listIcon#" index="i" >
													<cfoutput>
														<option value="#i#" data-icon="#i#"> <span style="margin-left:5px">#i#</span></option>
									...
									<div class="col-sm-12" style="box-shadow:0!important">
										<div class="form-group " >
											<label for="perfisAutorizados">Perfis Autorizados:</label>
											<select id="perfisAutorizados" required="true" class="form-control" multiple="multiple">
									...
									<div class="col-sm-6">
										<div class="form-group">
											...
											<label for="ordemExibicao" >N° Ordem de exibição:</label>
											<select id="ordemExibicao" required="false"  class="form-control icons_select2" >
												<option selected="" value=""></option>
	...
	<script language="JavaScript">
	  ...
	  function iformat(icon) {
		if (!icon.id) { return icon.text; }
			...
			return $('<span style="font-size:16px;"><i class="fa fa-2x ' + $(originalOption).data('icon') + '"></i> ' + icon.text + '</span>');
		}
		...
		$("#btSalvar").on('click', function (event)  {
			...
			if (!$('#controleNomeLink').val() || !$('#perfisAutorizados').val() || !$('#controlePagina').val() ){
				//mostra mensagem de erro, se algum campo necessário nesta fase não estiver preenchido	
				toastr.error('Preencha os campos obrigatórios: nome do link, perfis autorizados, página e n° ordem de exibição.');
				return false;
			}

			var mensagem = ""
	        if($('#controleId').val() == ''){
				var mensagem = "Deseja cadastrar este link?"
			}else{
				var mensagem = "Deseja editar este link?"
			}
			
			if(confirm(mensagem)){	
				var perfisList = $('#perfisAutorizados').val();
				if($('#ordemExibicao').val() ==''){
					var ordem = 1;
				}else{
					var ordem = $('#ordemExibicao').val();
				}
				$('#modalOverlay').modal('show');
					$.ajax({
						type: "POST",
						...
						data:{
							...
							pc_controle_acesso_nomeMenu: $('#controleNomeLink').val(),
							pc_controle_acesso_pagina: $('#controlePagina').val(),
							pc_controle_acesso_perfis: perfisList.join(','),
							pc_controle_acesso_grupoMenu: $('#controleNomeGrupo').val(),
							pc_controle_acesso_subgrupoMenu: $('#controleNomeSubgrupo').val(),
					...
					.done(function(result) {
						...
						$('#iconesGrupo').val(null).trigger('change');
		...
		function controleEditar(controleId,controleNomeLink,controlePagina,perfisAutorizados,controleNomeGrupo,controleNomeSubgrupo,iconesGrupo,iconesSubgrupo,iconesLink,ordemExibicao)  {
			...
			$('#cabecalhoAccordion').text("Editar o Link:" + ' ' + controleNomeLink);
			...
			$.each(listPerfisAutorizados, function(index,value){
				selectedValues[index] = value;
			});
			
			$('#perfisAutorizados').val(selectedValues).trigger('change');

			$('#cadastroLinks').CardWidget('expand')
			$('html, body').animate({ scrollTop: ($('#content').offset().top)} , 1000);

		}

		function controleExcluir(controleId){
			event.preventDefault()
			event.stopPropagation()
