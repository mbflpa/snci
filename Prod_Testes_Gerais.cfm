...
<cfif (grpacesso eq 'GESTORES') or (grpacesso eq 'DESENVOLVEDORES') or (grpacesso eq 'GESTORMASTER') or (grpacesso eq 'INSPETORES')>
	...
	<cfif IsDefined("FORM.submit") AND IsDefined("FORM.acao") And Form.acao is "Salvar">
		...
		<cfif IsDefined("FORM.frmcbarea") AND FORM.frmcbarea EQ "">
			...
			<cfelse>
				<cfquery datasource="#dsn_inspecao#">
					update Andamento set  And_DtPosic=#createodbcdate(CreateDate(Year(Now()),Month(Now()),Day(Now())))#,And_username='#CGI.REMOTE_USER#',And_HrPosic='000011'
					where And_NumInspecao='#FORM.ninsp#' and And_Unidade='#FORM.unid#' and And_NumGrupo=#FORM.ngrup# and And_NumItem=#FORM.nitem# and And_Situacao_Resp = 11
				</cfquery>			
			</cfif>
    	<cfelse>
			...
			<cfquery datasource="#dsn_inspecao#">
				UPDATE ParecerUnidade SET Pos_Situacao_Resp =
					...
					<cfelseif auxsitresp eq 26>
							...
							<cfset IDArea=#qVerificaArea.Rep_CodArea#>
							, Pos_Situacao = 'NC'
					...
					<cfelse>
							...
							<cfset IDArea=#rsOrgSub011.Rep_Codigo#>
							, Pos_Situacao = 'TS'
			...
			<cfset hhmmssdc = Replace(hhmmssdc,'.','',"All")>
			<cfset and_obs = DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '> ' & #Trim(Encaminhamento)#  & CHR(13) & CHR(13) & #Gestor# & CHR(13) & CHR(13) & #aux_obs# & CHR(13) & CHR(13) & 'Situação: ' & #situacao# & CHR(13) & CHR(13) & 'Responsável: ' & #maskcgiusu# & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '-----------------------------------------------------------------------------------------------------------------------'>
			<cfquery datasource="#dsn_inspecao#">
...
<cfif rsTPUnid.Und_TipoUnidade is 12 or rsTPUnid.Und_TipoUnidade is 16>
...
<body onLoad="exibevalores(); if(document.form1.houveProcSN.value != 'S') {exibe(document.form1.frmResp.value)} else {exibe(24)}; hanci(); controleNCI(); exibir_Area011(document.form1.nci.value);"> 
