<!--- versao anterior 
<cfset frmano = 2023>
<cfset dtlimit = CreateDate(2023,10,31)>
<cfset se='03'>
<cfset anoexerc = '2023'>--->
...
<cfquery name="qUsuario" datasource="#dsn_inspecao#">
	select Usu_GrupoAcesso, Usu_Matricula from usuarios where Usu_Matricula = '85051071'
</cfquery>
...
<!--- 	dtini#dtini# dtfim#dtfim# dtlimit: #dtlimit#
	<cfset gil = gil>   --->
 	...
 	<cfif rsVazio.recordcount lte 0>
		...
		<!--- CALCULAR OS DIAS ÚTEIS ENTRE A DATA NO STATUS 14-NR ATE O ÚLTIMO DIA DO MÊS DT_REFERENCIA --->
			
		<cfinclude template="INDICADORES_MES.CFM"> 
		<cfquery name="rsSLNC" datasource="#dsn_inspecao#">
			SELECT Andt_AnoExerc, Andt_Mes, Andt_TipoRel, Andt_CodSE, Und_TipoUnidade, Andt_Unid, Andt_Insp, Andt_Grp, Andt_Item, Andt_DPosic, Andt_HPosic, Andt_Resp, Andt_user, Andt_Area,
				Andt_NomeOrgCondutor, Andt_RespAnt, Andt_PosPontuacaoPonto, Andt_PosClassificacaoPonto, Andt_dtultatu
				FROM Andamento_Temp INNER JOIN Unidades ON Andt_Unid = Und_Codigo
				WHERE Andt_AnoExerc='#year(dtfim)#' AND 
				Andt_Mes=#month(dtfim)# AND 
				Andt_TipoRel=1 AND 
				Und_TipoUnidade <>12 And Und_TipoUnidade<>16
				ORDER BY Andt_Insp, Andt_Grp, Andt_Item, Andt_DPosic
		</cfquery>

		<cfoutput query="rsSLNC">
			<cfquery name="rsLiberado" datasource="#dsn_inspecao#">
				SELECT INP_DtEncerramento
				FROM Inspecao
				WHERE INP_Unidade='#rsSLNC.Andt_Unid#' AND 
					  INP_NumInspecao='#rsSLNC.Andt_Insp#' AND 
					  INP_DtEncerramento >= #dt30diasuteis14NR#
			</cfquery>
			
			<cfif rsLiberado.recordcount lte 0> 
				<cfset AndtDPosic = year(rsSLNC.Andt_DPosic) & '-' & month(rsSLNC.Andt_DPosic) & '-' & day(rsSLNC.Andt_DPosic)>
				<cfset dtultatu = year(rsSLNC.Andt_dtultatu) & '-' & month(rsSLNC.Andt_dtultatu) & '-' & day(rsSLNC.Andt_dtultatu)>

				<cfquery datasource="#dsn_inspecao#">
					insert into Andamento_Temp (Andt_AnoExerc, Andt_Mes, Andt_Insp, Andt_Unid, Andt_Grp, Andt_Item, Andt_DPosic, Andt_HPosic, Andt_Resp, Andt_tpunid, Andt_DiasCor,	Andt_Uteis, Andt_user, Andt_dtultatu, Andt_Area, Andt_NomeOrgCondutor, Andt_CodSE, Andt_DTRefer, Andt_Prazo, Andt_TipoRel, Andt_RespAnt, Andt_PosPontuacaoPonto, Andt_PosClassificacaoPonto) values ('#rsSLNC.Andt_AnoExerc#', #rsSLNC.Andt_Mes#, '#rsSLNC.Andt_Insp#', '#rsSLNC.Andt_Unid#', #rsSLNC.Andt_Grp#, #rsSLNC.Andt_Item#,'#AndtDPosic#', '#rsSLNC.Andt_HPosic#', #rsSLNC.Andt_Resp#,#rsSLNC.Und_TipoUnidade#, 0, 0, '#rsSLNC.Andt_user#', #dtultatu#, '#rsSLNC.Andt_Area#', '#trim(rsSLNC.Andt_NomeOrgCondutor)#', '#rsSLNC.Andt_CodSE#', #dtfim#, 'NN', 2, '#rsSLNC.Andt_RespAnt#', #rsSLNC.Andt_PosPontuacaoPonto#, '#rsSLNC.Andt_PosClassificacaoPonto#')
				</cfquery>
			</cfif> 
