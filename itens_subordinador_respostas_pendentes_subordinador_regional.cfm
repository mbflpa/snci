...
<cfquery name="qAcesso" datasource="#dsn_inspecao#">
	...
	<cfelse>	
		  <cfset sigla = 'TODOS OS ÓRGÃOS SUBORDINADOS À(AO) #qArea.Ars_Sigla#'>
	</cfif>
...
<cfif (Trim(qAcesso.Usu_GrupoAcesso) eq 'DESENVOLVEDORES') or (Trim(qAcesso.Usu_GrupoAcesso) eq 'GESTORES') or (Trim(qAcesso.Usu_GrupoAcesso) eq 'SUBORDINADORREGIONAL')>
    ...
    <cfquery name="rsItem" datasource="#dsn_inspecao#">
                   ...
                   <cfif ckTipo eq "1">
						SELECT  INP_DtInicInspecao, Pos_NumItem, Pos_Unidade, Pos_DtPrev_Solucao, Pos_DtPosic, Pos_ClassificacaoPonto, DATEDIFF(dd, Pos_DtPosic, GETDATE())
						AS Quant, Unidades.Und_Descricao, Unidades.Und_TipoUnidade, Pos_Inspecao, Pos_NumGrupo, Pos_dtultatu, DATEDIFF(dd,Pos_DtPosic,GETDATE()) AS data,
						...
						ON Rep_Codigo = RIP_CodReop AND right([Pos_Inspecao], 4) = Grp_Ano
						INNER JOIN Itens_Verificacao ON Itn_NumGrupo = Pos_NumGrupo AND Pos_NumItem = Itn_NumItem AND Grp_Ano = Itn_Ano and Itn_TipoUnidade = Und_TipoUnidade AND INP_Modalidade = Itn_Modalidade
						INNER JOIN Situacao_Ponto ON Pos_Situacao_Resp = STO_Codigo
						
						WHERE  pos_situacao_resp in (5,4,16,19,21) and (pos_area='#qAcesso.Usu_Lotacao#')     
						ORDER BY Und_Descricao, Pos_NumGrupo, Pos_NumItem, Pos_Inspecao, Quant
				 <cfelse>
          SELECT INP_DtInicInspecao, Pos_NumItem, Pos_Unidade, Pos_DtPrev_Solucao, Pos_DtPosic, Pos_ClassificacaoPonto, DATEDIFF(dd, Pos_DtPosic, GETDATE()) AS Quant, Unidades.Und_Descricao, Unidades.Und_TipoUnidade, Pos_Inspecao, Pos_NumGrupo, Pos_dtultatu,
                      Itn_Descricao, Grp_Descricao, Pos_Situacao_Resp, Reops.Rep_Codigo, Reops.Rep_Nome, Pos_Parecer, pos_area, 
                      INP_DtInicInspecao, INP_DtFimInspecao, INP_DtEncerramento, STO_Codigo, STO_Sigla, STO_Cor, STO_Descricao
            ...
            INNER JOIN Unidades 
                    ...
                    ON (Inspecao.INP_Unidade = ParecerUnidade.Pos_Unidade) AND (Inspecao.INP_NumInspecao = ParecerUnidade.Pos_Inspecao)) 
                    
            INNER JOIN (Itens_Verificacao  
            INNER JOIN Grupos_Verificacao 
                    ON Itens_Verificacao.Itn_NumGrupo = Grupos_Verificacao.Grp_Codigo) 
                    ON (ParecerUnidade.Pos_NumGrupo = Itens_Verificacao.Itn_NumGrupo) 
                       AND (ParecerUnidade.Pos_NumItem = Itens_Verificacao.Itn_NumItem)
                       AND right([Pos_Inspecao], 4) = Itens_Verificacao.Itn_Ano AND Itens_Verificacao.Itn_Ano = Grupos_Verificacao.Grp_Ano and Itn_TipoUnidade = Und_TipoUnidade AND (INP_Modalidade = Itn_Modalidade))
    ...
    <!---<cfset objPOI.WriteSingleExcel(
        ...
        Query = rsXLS,
        ColumnList = "INPModal,Und_CodDiretoria,Pos_Unidade,Und_Descricao,Pos_Inspecao,Pos_NumGrupo,Grp_Descricao,Pos_NumItem,Itn_Descricao,Und_CodReop,RIP_Ano,RIP_Resposta,comentario,RIP_Valor,RIP_Caractvlr,RIPFalta,RIPSobra,RIPEmRisco,POSDtUltAtu,POS_UserName,RIP_Recomendacao,INP_HrsPreInspecao,INPDtInicDesloc,INP_HrsDeslocamento,INPDtFimDesloc,INPDtInicInsp,INPDtFimInsp,INP_HrsInspecao,INP_Situacao,INPDtEncer,INP_Coordenador,INP_Responsavel,motivo,Pos_Situacao_Resp,STO_Sigla,STO_Descricao,PosDtPosic,PosDtPrevSoluc,Pos_Area,Pos_NomeArea,parecer,Pos_VLRecuperado,Pos_Processo,Pos_Tipo_Processo,Pos_SEI,RIPRInsp,RIPRGp,RIPRIt,Quant",
    ...
    <table width="100%" class="exibir">
     ...
     <td width="3%" align="center" bgcolor="eeeeee" class="exibir"><strong>Previsão Solução Até</strong></td>
     ...
     <td width="3%" align="center" bgcolor="eeeeee" class="exibir"><strong>Qtd. Dias</strong></td>
	 <td width="12%" align="center" bgcolor="eeeeee" class="exibir"></td>
     <td width="4%" align="center" bgcolor="eeeeee" class="exibir"><strong>Classificação</strong></td>
   </tr>
...
<cfif btnSN eq 'N'> 
      <cfif ckTipo eq "1">
        <td width="7%" bgcolor="#STO_Cor#"><div align="center"><a href="itens_subordinador_respostas_pendentes_subordinador_regional1.cfm?ckTipo=#URL.ckTipo#&Unid=#rsItem.Pos_Unidade#&Ninsp=#rsItem.Pos_Inspecao#&Ngrup=#rsItem.Pos_NumGrupo#&Nitem=#rsItem.Pos_NumItem#&Situacao=#rsItem.Pos_Situacao_Resp#&diasdecor=#data#" class="exibir" onMouseMove="Hint('#STO_Sigla#',2)" onMouseOut="Hint('#STO_Sigla#',1)"><strong>#trim(STO_Descricao)#</strong></a></div></td>
      <cfelse>
        <td width="7%" bgcolor="#STO_Cor#"><div align="center"><a target="_blank" href="itens_subordinador_respostas_pendentes_subordinador_regional1.cfm?ckTipo=#URL.ckTipo#&Unid=#rsItem.Pos_Unidade#&Ninsp=#rsItem.Pos_Inspecao#&Ngrup=#rsItem.Pos_NumGrupo#&Nitem=#rsItem.Pos_NumItem#&Situacao=#rsItem.Pos_Situacao_Resp#&diasdecor=#data#" class="exibir" onMouseMove="Hint('#STO_Sigla#',2)" onMouseOut="Hint('#STO_Sigla#',1)"><strong>#trim(STO_Descricao)#</strong></a></div></td>
      </cfif> 
<cfelse>
     <cfif ckTipo eq "1">
       <td width="7%" bgcolor="A80643"><div align="center"><a href="itens_subordinador_respostas_pendentes_subordinador_regional1.cfm?ckTipo=#URL.ckTipo#&Unid=#rsItem.Pos_Unidade#&Ninsp=#rsItem.Pos_Inspecao#&Ngrup=#rsItem.Pos_NumGrupo#&Nitem=#rsItem.Pos_NumItem#&Situacao=#rsItem.Pos_Situacao_Resp#&diasdecor=#data#" class="exibir" onMouseMove="Hint('#STO_Sigla#',2)" onMouseOut="Hint('#STO_Sigla#',1)"><strong>PERDA PRAZO UNIDADE</strong></a></div></td>
     <cfelse>
       <td width="7%" bgcolor="A80643"><div align="center"><a target="_blank" href="itens_subordinador_respostas_pendentes_subordinador_regional1.cfm?ckTipo=#URL.ckTipo#&Unid=#rsItem.Pos_Unidade#&Ninsp=#rsItem.Pos_Inspecao#&Ngrup=#rsItem.Pos_NumGrupo#&Nitem=#rsItem.Pos_NumItem#&Situacao=#rsItem.Pos_Situacao_Resp#&diasdecor=#data#" class="exibir" onMouseMove="Hint('#STO_Sigla#',2)" onMouseOut="Hint('#STO_Sigla#',1)"><strong>PERDA PRAZO UNIDADE</strong></a></div></td>
     </cfif> 
</cfif> 
	  ...
	  <td width="12%" class="red_titulo"><div align="center">#numReincInsp#</div></td>
