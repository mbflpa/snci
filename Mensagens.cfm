...
<cfloop condition="rotina lte 29">   
	<cfif rotina eq 1>
		...
		<cfquery name="rsAtuAND" datasource="#dsn_inspecao#">
			...
			FROM ParecerUnidade LEFT JOIN Andamento ON (Pos_Situacao_Resp = And_Situacao_Resp) AND (Pos_DtPosic = And_DtPosic) AND (Pos_NumItem = And_NumItem) AND (Pos_NumGrupo = And_NumGrupo) AND 
			(Pos_Unidade = And_Unidade) AND ([Pos_Inspecao] = [And_NumInspecao])
	...
	<cfif rotina eq 3 and rotinaSN is 'S'>
		...
		<cfquery name="rs14NR_2PU" datasource="#dsn_inspecao#">
	  	...
	  	<cfoutput query="rs14NR_2PU">
			...
			<cfset aux_obs = #rs14NR_2PU.Pos_Parecer# & CHR(13) & CHR(13) & DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '>' & #Trim(Encaminhamento)# & CHR(13) & CHR(13) & 'À(O) ' & #trim(aux_posnomearea)# & CHR(13) & CHR(13) & #sinformes# & CHR(13) & CHR(13) & 'Situação: ' & #rot2_DescStatus# & CHR(13) & CHR(13) & 'Responsável: SUBG CONTR INT OPER/GCOP' & CHR(13) & CHR(13) & '--------------------------------------------------------------------------------------------------------------'>
			<cfquery datasource="#dsn_inspecao#">
			UPDATE ParecerUnidade SET Pos_Area = '#aux_posarea#', Pos_NomeArea = '#aux_posnomearea#', Pos_Situacao_Resp = #rot2_IDStatus#, Pos_Situacao = '#rot2_SglStatus#', Pos_DtUltAtu = CONVERT(char, GETDATE(), 120), Pos_DtPosic = #dtatual#, Pos_DtPrev_Solucao = #dtatual#, Pos_Parecer = '#aux_obs#', Pos_Sit_Resp_Antes = 14 WHERE Pos_Unidade='#rs14NR_2PU.Pos_Unidade#' AND Pos_Inspecao='#rs14NR_2PU.Pos_Inspecao#' AND Pos_NumGrupo=#rs14NR_2PU.Pos_NumGrupo# AND Pos_NumItem=#rs14NR_2PU.Pos_NumItem#
			</cfquery>
	...
	<cfif rotina eq 6 and rotinaSN is 'S'>
		<cfoutput>Modulo de nº : #rotina# em execucao</cfoutput><br>
	   <cfquery name="rs141820_26NC" datasource="#dsn_inspecao#">
		 ...
		 INNER JOIN ParecerUnidade ON Und_Codigo = Pos_Unidade 
		 INNER JOIN Reops ON Und_CodReop = Rep_Codigo
		 WHERE Pos_Situacao_Resp in (14,18,20) and 
		...
		<cfoutput query="rs141820_26NC">
			   ...
			   <cfquery name="rsGERAT" datasource="#dsn_inspecao#">
				...
				FROM Areas 
				WHERE (left(Ars_Codigo,2) = '#left(rs141820_26NC.Pos_Area,2)#') AND (Ars_Descricao Like 'GER REG ATENDIMENTO%') and (Ars_Status = 'A') 
				ORDER BY Ars_Codigo
			  </cfquery>
			  ...
			  <cfelse>
				  <cfquery name="rsGEOPE" datasource="#dsn_inspecao#">
					SELECT Ars_Codigo, Ars_Descricao, Ars_Email
				    FROM Areas 
					WHERE (left(Ars_Codigo,2) = '#left(rs141820_26NC.Pos_Area,2)#') AND (Ars_Descricao Like 'GER REG OPERACAO/GEOPE%') and (Ars_Status = 'A') 
					ORDER BY Ars_Codigo
				  </cfquery>
				...
				<cfset sinformes = 'Em virtude da ausência de conclusão do Item da Avaliação pelo(a) Gestor(a) da unidade: ' & #trim(rs141820_26NC.Und_Descricao)# & CHR(13) & ', no prazo solicitado até ' & #DateFormat(rs141820_26NC.Pos_DtPrev_Solucao,"DD/MM/YYYY")# & ', estamos modificando para o Status de NÃO REGULARIZADO - APLICAR O CONTRATO e no aguardo do Aprimoramento para às devidas providências.'>
						 				
				<cfset aux_obs = #rs141820_26NC.Pos_Parecer# & CHR(13) & CHR(13) & DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '>' & #Trim(Encaminhamento)# & CHR(13) & CHR(13) & 'À(O) ' & #rot5_posnomearea# & CHR(13) & CHR(13) & #sinformes# & CHR(13) & CHR(13) & 'Situação: NÃO REGULARIZADO - APLICAR O CONTRATO' & CHR(13) & CHR(13) & 'Responsável: SUBG CONTR INT OPER/GCOP' & CHR(13) & '--------------------------------------------------------------------------------------------------------------'>
			  <cfquery datasource="#dsn_inspecao#">
			   UPDATE ParecerUnidade SET Pos_Situacao_Resp = 26, Pos_Situacao = 'NC', Pos_DtUltAtu = CONVERT(varchar, GETDATE(), 120), Pos_DtPosic = #dtatual#, Pos_DtPrev_Solucao = #dtatual#, Pos_Area = '#rot5_posarea#', Pos_NomeArea = '#rot5_posnomearea#', Pos_Parecer = '#aux_obs#', Pos_Sit_Resp_Antes = 18 WHERE Pos_Unidade='#rs141820_26NC.Pos_Unidade#' AND Pos_Inspecao='#rs141820_26NC.Pos_Inspecao#' AND Pos_NumGrupo=#rs141820_26NC.Pos_NumGrupo# AND Pos_NumItem=#rs141820_26NC.Pos_NumItem#
			 </cfquery>
			...
			<cfset hhmmssdc = Replace(hhmmssdc,'.','',"All")>				 
			 <cfquery datasource="#dsn_inspecao#">
			   insert into Andamento (And_NumInspecao, And_Unidade, And_NumGrupo, And_NumItem, And_DtPosic, And_username, And_Situacao_Resp, And_HrPosic, And_Parecer, And_Area) values ('#rs141820_26NC.Pos_Inspecao#', '#rs141820_26NC.Pos_Unidade#', #rs141820_26NC.Pos_NumGrupo#, #rs141820_26NC.Pos_NumItem#, convert(char, getdate(), 102), 'Rotina_Automatica', 26, '#hhmmssdc#', '#and_obs#', '#rot5_posarea#')
			 </cfquery>			
...
<!---    <cffile action="Append" file="#slocal##sarquivo#" output='#dateformat(now(),"DD/MM/YYYY HH:MM")#;Mensagens.cfm;rotina 21;aviso por e-mail aos Status 1-RU, 6-RA, 7-RS, 21-RV, 22-RO que Pos_DtPosic possui mais de 14 dias corridos'>	 --->   	  			  
	...
	<cfif rotina eq 23 and rotinaSN is 'S'>
			...
			<cfquery name="rsRotina23" datasource="#dsn_inspecao#">
				SELECT Dir_Codigo, Dir_Sigla 
				FROM Diretoria 
				where Dir_Codigo <> '01'
				ORDER BY Dir_Sigla
			</cfquery>
			<cfoutput query="rsRotina23">
				
				<cfquery name="rs23" datasource="#dsn_inspecao#">
				...
				(Und_CodDiretoria = '#rsRotina23.Dir_Codigo#') 
				ORDER BY Und_Descricao, Pos_Inspecao, Pos_NumGrupo, Pos_NumItem, Pos_DtPosic
				</cfquery>	

				<cfif rs23.recordcount gt 0>

				<cfset auxSE = rsRotina23.Dir_Codigo>
				<cfset scia_se = auxSE>
				...
				<cfelseif auxSE eq '22' OR auxSE eq '24'>   <!--- MS; MT --->
					...
					<cfset rot23_sdestina = rsSCIA.Ars_Email>
					<cfset rot23_sdestina = rot23_sdestina & ';' & rsSCIA.Ars_EmailCoordenador>
					
					<cfif findoneof("@", #trim(rot23_sdestina)#) eq 0>
						 <cfset rot23_sdestina = "gilvanm@correios.com.br">
					</cfif>
				
					<cfmail from="SNCI@correios.com.br" to="#rot23_sdestina#" subject="GESTORES - Em Análise" type="HTML">  
				...
				&nbsp;&nbsp;&nbsp;Para registro de suas Análises acesse o SNCI (endereço: http://intranetsistemaspe/snci/rotinas_inspecao.cfm) clicando no link: <a href="http://intranetsistemaspe/snci/rotinas_inspecao.cfm">SNCI</a><br><br>
						...
						<cfloop query="rs23">
							...
							<cfset auxdtnova = CreateDate(year(rs23.Pos_DtPosic),month(rs23.Pos_DtPosic),day(rs23.Pos_DtPosic))>
							<cfset auxDTFim = DateAdd("d", #rs23.diasocorr#, #auxdtnova#)>
							...
							<cfloop condition="nCont lte #rs23.diasocorr#">
								...
								<cfset dduteis = #rs23.diasocorr# - #dsusp#> 
							<!---  --->
							<cfif (dduteis gte 20)> 					
								<tr>
									<td align="left"><strong>#rs23.Pos_Unidade#</strong></td>
									<td align="left"><strong>#rs23.Und_Descricao#</strong></td>
									<td align="center"><strong>#rs23.Pos_Inspecao#</strong></td>
									<td align="center"><strong>#rs23.Pos_NumGrupo#</strong></td>
									<td align="center"><strong>#rs23.Pos_NumItem#</strong></td>
									<td align="center"><strong>#dateformat(rs23.Pos_DtPosic,"dd/mm/yyyy")#</strong></td>
									<td align="center"><strong>#rs23.STO_Sigla#</strong></td>
									<td align="left"><strong>#trim(rs23.STO_Descricao)#</strong></td>
									<cfset auxposic = DateFormat(now(),"YYYY/MM/DD")>
									<cfset DIAS = dduteis>
									<td align="right"><strong>#DIAS#</strong></td>
								</tr>
...
<!--- ************** MUDAR STATUS conforme Demanda nº 58 ****************** --->
	...
	<cfif rotina eq 24 and rotinaSN is 'S'>
	  ...
	  <cfoutput query="rs10PS_TRAT">
			...
			<cfif rsExiste.recordcount gt 0>
				<cfif rs10PS_TRAT.Und_TipoUnidade is 12 || rs10PS_TRAT.Und_TipoUnidade is 16>
				   <!--- TERCEIRIZADA --->
				   <cfset rot24_StatusID = 18>
				   <cfset rot24_StatusSGL = 'TF'>
				   <cfset rot24_StatusDESC = 'TRATAMENTO TERCEIRIZADA'>
				 <cfelse>
				   <!--- Proprias --->
				   <cfset rot24_StatusID = 15>
				   <cfset rot24_StatusSGL = 'TU'>
				   <cfset rot24_StatusDESC = 'TRATAMENTO UNIDADE'>
				 </cfif>
			...
			<cfelse>
				...
				<cfif rsExiste.recordcount gt 0>
					<cfset rot24_StatusID = 19>
					<cfset rot24_StatusSGL = 'TA'>
					<cfset rot24_StatusDESC = 'TRATAMENTO AREA'>
				...
				<cfelse>
					...
					<cfif rsExiste.recordcount gt 0>
						<cfset rot24_StatusID = 16>
						<cfset rot24_StatusSGL = 'TS'>
						<cfset rot24_StatusDESC = 'TRATAMENTO ORGAO SUBORDINADOR'>
					...
					<cfelse>
						...
						<cfif rsExiste.recordcount gt 0>
							<cfset rot24_StatusID = 23>
							<cfset rot24_StatusSGL = 'TO'>
							<cfset rot24_StatusDESC = 'TRATAMENTO SUPERINTENDENCIA ESTADUAL'>
	 ...
	 <cfset aux_obs = #rs10PS_TRAT.Pos_Parecer# & CHR(13) & CHR(13) & DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '>' & #Trim(Encaminhamento)# & CHR(13) & CHR(13) & 'À(O) ' & #trim(aux_posnomearea)# & CHR(13) & CHR(13) & #sinformes# & CHR(13) & CHR(13) & 'Situação: ' & #rot24_StatusDESC# & CHR(13) & CHR(13) & 'Data de Previsão da Solução: ' & #DateFormat(PosDtPrevSolucao,"DD/MM/YYYY")# & CHR(13) & CHR(13)& CHR(13) & CHR(13) & 'Responsável: SUBG CONTR INT OPER/GCOP' & CHR(13) & CHR(13) & '--------------------------------------------------------------------------------------------------------------'>
		 <cfquery datasource="#dsn_inspecao#">
		   UPDATE ParecerUnidade SET Pos_Area = '#aux_posarea#', Pos_NomeArea = '#aux_posnomearea#', Pos_Situacao_Resp = #rot24_StatusID#, Pos_Situacao = '#rot24_StatusSGL#', Pos_DtUltAtu = CONVERT(char, GETDATE(), 120), Pos_DtPosic = convert(char, getdate(), 102), Pos_DtPrev_Solucao = #PosDtPrevSolucao#, Pos_Parecer = '#aux_obs#', Pos_Sit_Resp_Antes = 10 WHERE Pos_Unidade='#rs10PS_TRAT.Pos_Unidade#' AND Pos_Inspecao='#rs10PS_TRAT.Pos_Inspecao#' AND Pos_NumGrupo=#rs10PS_TRAT.Pos_NumGrupo# AND Pos_NumItem=#rs10PS_TRAT.Pos_NumItem#
		 </cfquery>
		 <cfset and_obs = DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '>' & #Trim(Encaminhamento)# & CHR(13) & CHR(13) & 'À(O) ' & #trim(aux_posnomearea)# & CHR(13) & CHR(13) & #sinformes# & CHR(13) & CHR(13) & 'Situação: ' & #rot24_StatusDESC# & CHR(13) & CHR(13) & 'Responsável: SUBG CONTR INT OPER/GCOP' & CHR(13) & CHR(13) & '--------------------------------------------------------------------------------------------------------------'>
			<cfset hhmmssdc = timeFormat(now(), "HH:MM:ssl")>
			<cfset hhmmssdc = Replace(hhmmssdc,':','',"All")>
			<cfset hhmmssdc = Replace(hhmmssdc,'.','',"All")>		 
		 <cfquery datasource="#dsn_inspecao#">
		   insert into Andamento (And_NumInspecao, And_Unidade, And_NumGrupo, And_NumItem, And_DtPosic, And_username, And_Situacao_Resp, And_HrPosic, And_Parecer, And_Area) values ('#rs10PS_TRAT.Pos_Inspecao#', '#rs10PS_TRAT.Pos_Unidade#', #rs10PS_TRAT.Pos_NumGrupo#, #rs10PS_TRAT.Pos_NumItem#, convert(char, getdate(), 102), 'Rotina_Automatica', #rot24_StatusID#, '#hhmmssdc#', '#and_obs#', '#aux_posarea#')
		 </cfquery>
	</cfoutput> 
