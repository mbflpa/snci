...
<cfquery name="qItemReanalise" datasource="#dsn_inspecao#">
	...
	WHERE Itn_Ano=RIGHT('#URL.Ninsp#',4) AND Grp_Ano=Itn_Ano and Itn_NumGrupo='#url.Ngrup#' and Itn_NumItem ='#url.Nitem#' and Itn_TipoUnidade = #url.tpunid#
</cfquery>
...
<cfif isDefined("Form.acao") and "#form.acao#" eq 'cancelarReanalise'>
		...
		<cfquery datasource="#dsn_inspecao#" name="rsVerificaItem">
			SELECT  RIP_Resposta, RIP_Unidade, RIP_NCISEI, RIP_Falta FROM Resultado_Inspecao 
		...
		<cfquery name="rsItem2" datasource="#dsn_inspecao#">
			SELECT Itn_TipoUnidade, Itn_Pontuacao, Itn_Classificacao 
			FROM Itens_Verificacao 
			WHERE (Itn_Ano = right('#FORM.Ninsp#',4)) and (Itn_NumGrupo = '#FORM.Ngrup#') AND (Itn_NumItem = '#FORM.Nitem#') and (Itn_TipoUnidade = #url.tpunid#)
		</cfquery>
...
<!--- inicio classificacao do ponto --->
					...
					<cfset auxfrmfalta = Replace(rsVerificaItem.RIP_Falta,',','.','All')>
					<cfif auxfrmfalta neq '0.00'>
							...
							<cfset VLR_Fin_Ponto = NumberFormat((Fator_VLR_Fin * #rsItem2.Itn_Pontuacao#),999)>
	...
	<!--- fim classificacao do ponto --->	
			<cfquery datasource="#dsn_inspecao#">
				...
				VALUES ('#FORM.unid#', '#FORM.Ninsp#', #FORM.Ngrup#, #FORM.Nitem#, 
						...
						'#posarea_cod#','#posarea_nome#','#rsVerificaItem.RIP_NCISEI#', #rsItem2.Itn_Pontuacao#,'#ClasItem_Ponto#')
			</cfquery>
		...
		<!---Início do processo de liberação da avaliação--->
			...
			<cfquery datasource="#dsn_inspecao#" name="qVerifValidados">
				SELECT RIP_Resposta, RIP_Recomendacao,Itn_ValidacaoObrigatoria FROM Resultado_Inspecao 
				INNER JOIN Itens_Verificacao ON Itn_Ano = RIP_Ano AND Itn_NumGrupo = RIP_NumGrupo AND Itn_NumItem = RIP_NumItem and (Itn_TipoUnidade = #url.tpunid#)
			...
			<!---Início - Se não existirem itens em revisão e não existirem itens em reavaliação,e nao existirem itens a serem validados,
				...
				<cfif qInspecaoLiberada.recordCount eq 0 and qVerifEmReanalise.recordCount eq 0 and qVerifValidados.recordCount eq 0>
					...
					<cfquery name="rs11" datasource="#dsn_inspecao#">
						...
						FROM Itens_Verificacao 
						INNER JOIN (ParecerUnidade INNER JOIN Unidades ON Pos_Unidade = Und_Codigo) ON (Itn_Ano = #right(ninsp,4)# and Itn_NumItem = Pos_NumItem) AND (Itn_NumGrupo = Pos_NumGrupo) and (Itn_TipoUnidade = #url.tpunid#)
							...
							<cfif rsEmail.recordcount gt 0>
								...
								<cfset sdestina = #sdestina# & ';fsobrinho@correios.com.br'>
									...
									<cfquery name="rsSCOIEmail" datasource="#dsn_inspecao#">
										...
										(Ars_Sigla Like '%CCOP/SCOI%') AND (Ars_Status='A')
									</cfquery>
				...
				<!---Início - Se não existirem itens em revisão e não existirem itens em reavaliação e nao existirem itens a serem validados,
					...
					<cfif qInspecaoLiberada.recordCount eq 0 and qVerifEmReanalise.recordCount eq 0 and qVerifValidados.recordCount eq 0>
						...
						<cfquery name="rs11" datasource="#dsn_inspecao#">
							SELECT Und_TipoUnidade, Und_Centraliza, Itn_TipoUnidade, Pos_Unidade, Pos_NumGrupo, Pos_NumItem, Pos_Area, Pos_NomeArea 
							FROM Itens_Verificacao 
							INNER JOIN (ParecerUnidade INNER JOIN Unidades ON Pos_Unidade = Und_Codigo) ON (Itn_Ano = '#right(ninsp)#' and Itn_NumItem = Pos_NumItem) AND (Itn_NumGrupo = Pos_NumGrupo) and (Itn_TipoUnidade = #url.tpunid#)
...
SELECT RIP_Resposta, RIP_Recomendacao,Itn_ValidacaoObrigatoria FROM Resultado_Inspecao 
	INNER JOIN Itens_Verificacao ON Itn_Ano = RIP_Ano AND Itn_NumGrupo = RIP_NumGrupo AND Itn_NumItem = RIP_NumItem
	WHERE ((RTRIM(RIP_Resposta)= 'E' AND Itn_ValidacaoObrigatoria=1) OR RTRIM(RIP_Resposta)= 'V') AND   RTRIM(RIP_Recomendacao) IS NULL AND RIP_NumInspecao='#ninsp#'  
</cfquery>
