...
Und_Codigo = RIP_Unidade 
INNER JOIN Itens_Verificacao ON Grp_Codigo = Itn_NumGrupo and Grp_Ano = Itn_Ano and convert(char(4),RIP_Ano) = Itn_Ano
and Itn_TipoUnidade = Und_TipoUnidade AND INP_Modalidade = Itn_Modalidade and RIP_NumGrupo = Itn_NumGrupo and RIP_NumItem = Itn_NumItem
...
<cfelse>
	<cfset maskcgiusu = left(maskcgiusu,12) & mid(maskcgiusu,13,4) & '***' & right(maskcgiusu,1)>	
		...
		<cfif Form.frmResp eq 2>
		   <!--- resp = 2-PU --->
	       <cfset dtnovoprazo = DateAdd("d", 10, #dtnovoprazo#)>
        </cfif> 
		<cfif Form.frmResp eq 16>
		<!--- resp = 16-TS --->
	   ...
	   UPDATE ParecerUnidade SET Pos_Situacao_Resp = #FORM.frmResp#
		...
		<cfcase value=7>
		  <cfquery name="rsReop" datasource="#dsn_inspecao#">
		    SELECT Rep_Codigo, Rep_Nome FROM Reops WHERE Rep_Codigo = '#qUsuario.Usu_Lotacao#'
          </cfquery>
		  , Pos_Situacao = 'RS'
		  , Pos_Area = '#rsReop.Rep_Codigo#'
		  , Pos_NomeArea = '#rsReop.Rep_Nome#'
		  , Pos_DtPrev_Solucao = #createodbcdate(createdate(year(dtnovoprazo),month(dtnovoprazo),day(dtnovoprazo)))#
		  <cfquery name="rsCVCO" datasource="#dsn_inspecao#">
            SELECT Ars_Codigo, Ars_Sigla FROM Usuarios INNER JOIN Areas ON Usu_Lotacao = Ars_Codigo WHERE Usu_Matricula='#qInspetor.IPT_MatricInspetor#'
          </cfquery>
	  ...
	  , Pos_Parecer=
		...
		<!---	 <cfset aux_obs = Replace(aux_obs,'&','','All')>
				 <cfset aux_obs = Replace(aux_obs,'%','','All')> --->
		<cfset pos_aux = Form.H_obs & CHR(13) & CHR(13) & DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '> ' & Trim(Encaminhamento) & CHR(13) & CHR(13) & 'Á(o)  ' & #Gestor# & CHR(13) & CHR(13) & #aux_obs# & CHR(13) & CHR(13) & 'Data de Previsão da Solução: ' & #DateFormat(dtnovoprazo,"DD/MM/YYYY")# & CHR(13) & CHR(13) & 'Situação: ' & situacao & CHR(13) & CHR(13) &  'Responsável: ' & #maskcgiusu# & '\' & Trim(qUsuario.Usu_Apelido) & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '-----------------------------------------------------------------------------------------------------------------------'>
	  '#pos_aux#'
...
<cfquery name="qResposta" datasource="#dsn_inspecao#">
  SELECT Pos_NumGrupo, Pos_NumItem, Pos_Unidade, Pos_Situacao_Resp, Pos_Parecer, Pos_DtPrev_Solucao
  FROM ParecerUnidade
   ...
   if(strmanifesto.length < 100)
	...
	return false;
	 ...
	 {  
	   ...
	   {
	   if (sit != 16) {document.form1.cbData.disabled = true;}
	   return false;
	   }	
   }
//return false;
...
<!--- �rea de conte�do   --->
    <form name="form1" method="post" onSubmit="return validaForm()" enctype="multipart/form-data" action="itens_subordinador_respostas_pendentes_regional1.cfm">
      <div align="right"><table width="74%" align="center">
         ...
         <!---    <input type="hidden" name="frmResp" value="6"> --->
          ...
          <tr>
            <td valign="middle" bgcolor="eeeeee" class="exibir"><span class="titulos">Hist&oacute;rico:</span> <span class="titulos">Manifesta&ccedil;&atilde;o/An&aacute;lise do Controle Interno</span><span class="titulos">:</span></td>
		  ...
		  <tr>
        <td colspan="6" class="exibir" bgcolor="#eeeeee"><strong>ANEXOS</strong></td>
        <td colspan="2" bgcolor="#eeeeee" class="exibir"></td>
      </tr>
         
          <tr>
        <td bgcolor="eeeeee" class="exibir"><strong>Arquivo:</strong></td>
        <td colspan="5" bgcolor="eeeeee" class="exibir"><input name="arquivo" class="botao" type="file" size="50"></td>
	   ...
	   <tr>
            <td colspan="7" bgcolor="eeeeee" class="exibir">Situa&ccedil;&atilde;o:&nbsp;&nbsp;
              <select name="frmResp" class="form" id="frmResp" onChange="dtprazo(this.value)">
 ...
 <!--- ===================== --->	
	  ...
	  <cfif (dateformat(dtposicfut,'YYYYMMDD') gt DateFormat(dezdiasfutrat,'YYYYMMDD'))>
	  		<cfset dtposicfut = dezdiasfutrat>
	  </cfif>
	...
	<input name="cbData" id="cbData" type="text" class="form" onKeyPress="numericos()" onKeyDown="Mascara_Data(this)" size="14" maxlength="10" value="<cfoutput>#dtposicfut#</cfoutput>" onChange="aviso()" disabled></td>
         </tr><br><br>
          <tr>
            <td colspan="6"><div align="right">
                  <input name="button" type="button" class="botao" onClick="window.open('itens_subordinador_respostas_pendentes_subordinador_regional.cfm','_self')" value="Voltar">
            </div></td>
			<cfset resp= qResposta.Pos_Situacao_Resp>
			<cfif (resp eq 7)>
            	<td>&nbsp;&nbsp;&nbsp;&nbsp; <input name="Submit" type="submit" class="botao" value="Salvar"  onClick="document.form1.acao.value='Salvar';" disabled></td>
			<cfelse>
				<td>&nbsp;&nbsp;&nbsp;&nbsp;  <input name="Submit" type="submit" class="botao" value="Salvar"  onClick="document.form1.acao.value='Salvar';"></td>			
			</cfif>
          </tr>
        </table>
