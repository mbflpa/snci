               ...
               <!--- Catch error if MIME type is not accepted --->
                <cftry>
                    ...
                    <cfcatch type="any">
                         <script>
                         <cfif '#form.dir#' eq 'imagens'>
                              var erro = "ERRO: O tipo de arquivo carregado (<cfoutput>#cfcatch.MimeType#</cfoutput>) não foi aceito pelo servidor.\n\nSomente arquivos do tipo gif, jpeg, jpg e png podem ser carregados.\n\nVerifique se você está enviando um arquivo do tipo apropriado. ";
                              alert(erro);
                         <cfelse>
                ...
                <cfif isDefined("uploadResult")>
                    <script>
                    //     alert("Imagem encaminhada com sucesso ao Servidor!\n\nClique na imagem para adicioná-la ao editor de texto.\n\nObs.: A última imagem enviada sempre aparecerá na primeira fila à esquerda.")
	...
	<title>SNCI - UPLOAD DE IMAGENS</title>
     ...
     if(window.opener.closed) {  
          ...
          function EnviarArquivoOrientacoes(){
               if (document.getElementById("uploadFile").value != ''){
                    var grupo = window.opener.document.getElementById('selAltItemGrupo').value;
                    var item = window.opener.document.getElementById('selAltItem').value;
                    <cfoutput>
                          var edt = '#url.editor#';
                          var dir = '#url.dir#';
                    </cfoutput>
          ...
          function ExcluirImagem(file, name){ 
               if(window.confirm('Confirma a exclusão desta imagem?\n'+ name)){
          ...
          function CopiaUrl(url){//não utilizado neste form
          ...
          function InsereImagem(img,largura){
               ...
               <cfif '#url.editor#' neq 'Melhoria' >
                    ...
                    var alinhamento ='';
                    var radios = document.getElementsByName('imgAlinhamento');
                    for (var i = 0, length = radios.length; i < length; i++) {
                         if (radios[i].checked) {
                              alinhamento = radios[i].value;
                              break;
                         }
                    }
                   ...
                   if(dir == 'imagens'){
                         if(alinhamento == 'left'){
                              imagemHtml = '<figure style="float:left;"><div align="left">Informe um título aqui...</div><img style="cursor:pointer;float:left;" onclick="' + newwindow  + '" src=' + img + ' width="'+ largura + '" ></figure>';
                         }
                         if(alinhamento == 'center'){
                              imagemHtml = '<figure><div align="center">Informe um título aqui...</div><div align="center"><img onclick="' + newwindow  + '" src=' + img + ' width="'+ largura + '"  style="cursor:pointer;display: block;margin-left: auto;margin-right: auto"></div></figure><br>';
                         }
                         if(alinhamento == 'right'){
                              imagemHtml = '<figure style="float:right;"><div align="right">Informe um título aqui...</div><img style="cursor:pointer;float:right;" onclick="' + newwindow  + '" src=' + img + ' width="'+ largura + '" ></figure>';
                         }
                    }else{
                         if(alinhamento == 'left'){
                              imagemHtml = '<figure style="float:left;margin:0px"><img style="float:left;"  src=' + img + ' width="'+ largura + '" ></figure>';
                         }
                         if(alinhamento == 'center'){
                              imagemHtml = '<figure><div align="center"><img src=' + img + ' width="'+ largura + '"  style="display: block;margin-left: auto;margin-right: auto"></div></figure><br>';
                         }
                         if(alinhamento == 'right'){
                              imagemHtml = '<figure style="float:right;margin:0px"><img style="float:right;"  src=' + img + ' width="'+ largura + '" ></figure>';
                         }
                    }
               <cfelse>
                   imagemHtml = '<figure><div align="center">Informe um título aqui...</div><div align="center"><img onclick="' + newwindow  + '" src=' + img + ' width="'+ largura + '"  style="cursor:pointer;display: block;margin-left: auto;margin-right: auto"></div></figure><br><br>';     
               </cfif>
...
<body id="main_body"  style="background:#fff">
     ...
     name="formListFile" style="width:714px" class="appnitro" enctype="multipart/form-data" method="post" >
          ...
          <cfif '#url.editor#' eq 'Melhoria'>
               ...
               <cfelse>
                    <div style="left:20px">O número máximo de imagens enviadas ao servidor para este item foi atingido.</div>
