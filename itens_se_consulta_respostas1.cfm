...
<cfquery name="qAcesso" datasource="#dsn_inspecao#">
	select Usu_GrupoAcesso, Usu_Lotacao, Usu_DR from usuarios where Usu_login = (<cfqueryparam cfsqltype="cf_sql_varchar" value="#cgi.REMOTE_USER#">)
</cfquery>
<cfquery name="qSE" datasource="#dsn_inspecao#">
   SELECT Dir_Descricao, Dir_Sigla FROM Diretoria WHERE Dir_Codigo='#qAcesso.Usu_DR#'
</cfquery>
...
<cfquery name="rsItem" datasource="#dsn_inspecao#">
 SELECT Pos_Situacao_Resp, Pos_Parecer, RIP_Recomendacoes, RIP_Comentario, RIP_Caractvlr, RIP_Falta, RIP_Sobra, RIP_EmRisco, RIP_Valor, Dir_Descricao, Dir_Codigo, Pos_Area, Pos_Processo, Pos_Tipo_Processo, Pos_Abertura, Itn_Descricao, Pos_VLRecuperado, Pos_DtPrev_Solucao, Pos_DtPosic, DATEDIFF(dd,Pos_DtPosic,GETDATE()) AS diasOcor, Pos_SEI, INP_DtInicInspecao, Grupos_Verificacao.Grp_Descricao
FROM ParecerUnidade
...
<cfelse>
  ...
  <cfset sSigla_Resp = "RO">
  <cfset sDesc_Resp = "RESPOSTA SUPERINTENDENCIA">
  <cfquery datasource="#dsn_inspecao#">
		...
		<!--- Valor pre-existente em banco ref. ao campo (Pos_Situacao_Resp => 14 = N�O RESPONDIDO)  --->
		 <!--- Valor digitado com a data de agora (now()) --->
		  ...
		  <cfelse>
				 ...
				 <cfset Encaminhamento = 'À SUPERINTENDENCIA'>
				 <cfset nID_Resp = 23>
				 <cfset sSigla_Resp = "TO">
				 <cfset sDesc_Resp = "TRATAMENTO SUPERINTENDENCIA">
		  </cfif>
  ...
  , Pos_Parecer=
	...
	<!---	 <cfset aux_obs = Replace(aux_obs,'&','','All')>
		     <cfset aux_obs = Replace(aux_obs,'%','','All')> --->
  <cfset pos_obs = trim(Form.H_obs) & CHR(13) & DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '>' & Trim(Encaminhamento) & CHR(13) & CHR(13) & #aux_obs# & CHR(13) & CHR(13) & 'Situa��o: ' & #sDesc_Resp# & CHR(13) & CHR(13) & #data# & #DateFormat(dtcbData,"DD/MM/YYYY")# & CHR(13) & CHR(13) & 'Respons�vel: ' & #maskcgiusu# & '\' & Trim(qUsuario.Usu_Apelido) & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '--------------------------------------------------------------------------------------------------------------'>
	'#pos_obs#'
   </cfif>
 ...
 <cfset and_obs = DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '>' & Trim(Encaminhamento) & CHR(13) & CHR(13) & #aux_obs# & CHR(13) & CHR(13) & 'Situação: ' & #sDesc_Resp# & CHR(13) & CHR(13) & #data# & #DateFormat(dtcbData,"DD/MM/YYYY")# & CHR(13) & CHR(13) & 'Respons�vel: ' & #maskcgiusu# & '\' & Trim(qUsuario.Usu_Apelido) & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '--------------------------------------------------------------------------------------------------------------'>
...
<cfquery name="qResposta" datasource="#dsn_inspecao#">
  SELECT Pos_NumGrupo, Pos_NumItem, Pos_Unidade, Pos_Situacao_Resp, Pos_Parecer, Pos_DtPrev_Solucao, Pos_DtPosic
  FROM ParecerUnidade
...
function salvar(x){
	...
	if(strmanifesto == '')
     ...
     if(strmanifesto.length < 100)
	  {
	   alert('Sr. Usu�rio! Sua manisfesta��o dever� conter no m�nimo 100(cem) caracteres');
	   return false;
	  }

	 if (dtprevdig.length != 10){
		alert("Preencher campo: Data da Previs�o da Solu��o ex. DD/MM/AAAA");
		return false;
	 }
...
<!--- 	 var pos_dtposic_amd_atual = document.form1.pos_posic_ymd.value; --->
	 ...
	 {
	  alert("Data de Previs�o da Solu��o � inferior a data de hoje!")
	  return false;
	 }
...
<!--- �rea de conte�do   --->
    <form name="form1" method="post" onSubmit="if (document.form1.acao.value == 'Salvar2') {return salvar(vPSitResp.value)}" enctype="multipart/form-data" action="itens_unidades_controle_respostas1_area.cfm">
      <div align="right"><table width="74%" align="center">
          <tr><br>
              <td colspan="9"><p align="center" class="titulo1"><strong> </strong><strong class="titulo1"><cfoutput>#qSE.Dir_Descricao# - SE/#qSE.Dir_Sigla#</cfoutput></strong>
		  ...
		  <tr class="exibir">
           ...
           <td colspan="7" bgcolor="f7f7f7"><cfoutput>#rsItem.RIP_Valor#</cfoutput></td>
          </tr>
          <tr class="exibir">
          ...
          <tr>
            <td valign="middle" bgcolor="eeeeee" class="exibir"><div align="center"><span class="titulos">Situa��o Encontrada:</span></div></td>
		 ...
		 <tr>
            <td bgcolor="eeeeee"><div align="center"><span class="exibir"><span class="titulos">Orienta��es:</span></span></div></td>
...
<cfoutput>
		...
		<cfinvoke component="#dao#" method="VencidoPrazo_Andamento" returnVariable="VencidoPrazo_Andamento" NumeroDaInspecao="#ninsp#" 
			CodigoDaUnidade="#unid#" Grupo="#ngrup#" Item="#nitem#" MostraDump="no" RetornaQuantDias="yes" ListaDeStatusContabilizados="8,23">
			
<!--- 			 <br><cfoutput>tempo #VencidoPrazo_Andamento#</cfoutput><br>  
			<cfset gil = gil>
		   <cfset sdestina = "">--->
		    
<!---    <cfquery name="qStatus8" datasource="#dsn_inspecao#">
		  select And_DtPosic, DATEDIFF(d, And_DtPosic, GETDATE()) as qtddias from andamento where And_Situacao_Resp = 8 and And_Unidade = '#unid#' and And_NumInspecao = '#ninsp#' and And_NumGrupo = #ngrup# and And_NumItem = #nitem# order by And_DtPosic desc, And_HrPosic desc
		</cfquery>
		</cfquery> --->
