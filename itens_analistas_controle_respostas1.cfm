...
<cfset grpacesso = ucase(Trim(qAcesso.Usu_GrupoAcesso))>
 ...
 <cfif (grpacesso neq 'GESTORES') and (grpacesso neq 'DESENVOLVEDORES') and (grpacesso neq 'GESTORMASTER') and (grpacesso neq 'INSPETORES') and (grpacesso neq 'ANALISTAS')>
	  <cfinclude template="aviso_sessao_encerrada.htm">
	  <cfabort>  
</cfif>  
...
<!--- <cfquery name="rsFator" datasource="#dsn_inspecao#">
	SELECT VLR_Fator, VLR_FaixaInicial, VLR_FaixaFinal 
	FROM ValorRelevancia where VLR_Ano = '#right(ninsp,4)#'
</cfquery> --->

<cfif (grpacesso eq 'GESTORES') or (grpacesso eq 'DESENVOLVEDORES') or (grpacesso eq 'GESTORMASTER') or (grpacesso eq 'INSPETORES')>

<!---  <cftry>  --->
  <cfif isDefined("Form.acao") And (Form.acao is 'alter_reincidencia' Or Form.acao is 'Excluir_Proc' Or Form.acao is 'Incluir_Proc' Or Form.acao is 'Excluir_Sei' Or Form.acao is 'Incluir_Causa' Or Form.acao is 'Anexar' Or Form.acao is 'Excluir_Anexo' Or Form.acao is 'Excluir_Causa' or Form.acao is 'alter_valores')>
...
<!--- Alterar reincidencia --->
 <cfif Form.acao is 'alter_reincidencia'>
	 
	 <cfif FORM.frmReinccb eq "S">
		 
		 <cfquery name="rsRIP_Existe" datasource="#dsn_inspecao#">
			 SELECT RIP_Unidade, RIP_NumInspecao, RIP_NumGrupo, RIP_NumItem
			 FROM Resultado_Inspecao 
			 WHERE RIP_Unidade='#FORM.unid#' AND 
			 RIP_NumInspecao='#FORM.frmreincInsp#' AND 
			 RIP_NumGrupo=#FORM.frmreincGrup# AND 
			 RIP_NumItem=#FORM.frmreincItem# and
			 Right(#FORM.ninsp#,4) > Right(#FORM.frmreincInsp#,4)
		 </cfquery>
     </cfif>
     <cfif rsRIP_Existe.recordcount gt 0>
...
<!--- 	
	...
	<cfif trim(form.caracvlr) eq 'Quantificado'>
   ...
   <cfif rsSitAtual.Pos_Situacao_Resp neq 0 and rsSitAtual.Pos_Situacao_Resp neq 11>
	...
	<cfset pos_aux = Form.H_obs & CHR(13) & CHR(13) & DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '> Alteracao de Valores' & CHR(13) & CHR(13) & #aux_obs# & CHR(13) & CHR(13) & ' Antes(R$) - Falta: ' & #form.sfrmfalta# & '  Sobra: ' & #form.sfrmsobra# & '  Em Risco: ' & #form.sfrmemrisco# & CHR(13) & 'Atual (R$) - Falta: ' & #form.frmfalta# & '  Sobra: ' & #form.frmsobra# & '  Em Risco: ' & #form.frmemrisco# & CHR(13) & CHR(13) &  'Data de Atualizacao: ' & #DateFormat(now(),"DD/MM/YYYY")# & CHR(13) & CHR(13) & 'Responsável: ' & #CGI.REMOTE_USER# & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '-----------------------------------------------------------------------------------------------------------------------'>
     <cfquery datasource="#dsn_inspecao#">
	...
	<cfif rsAnd.recordcount gt 0>
		...
		<cfif (auxfrmfalta neq '0.00') and (listFind('10','#form.ItnPTCSeq#') neq 0)>
			...
			<cfquery name="rsPtoMax" datasource="#dsn_inspecao#">
				...
				WHERE TUP_Ano = '#right(ninsp,4)#' AND TUP_Tun_Codigo = #sfrmTipoUnidade#
			</cfquery>

			<!--- Obter a Fator_VLR_Fin conforme o auxfrmfalta --->
			<cfset Fator_VLR_Fin = 1>
			<cfloop query="rsFator">
				<cfif (auxfrmfalta gt rsFator.VLR_FaixaInicial) and (auxfrmfalta lte rsFator.VLR_FaixaFinal)>
					<cfset Fator_VLR_Fin =  rsFator.VLR_Fator>		
				<cfelseif (rsFator.VLR_FaixaFinal is '0.00') and (auxfrmfalta gt rsFator.VLR_FaixaInicial)>
					<cfset Fator_VLR_Fin = rsFator.VLR_Fator>		
				</cfif>
			</cfloop>	
		
			<!--- Obter a VLR_Fin_Ponto --->
			<cfset VLR_Fin_Ponto = NumberFormat((Fator_VLR_Fin * #form.PosPontuacaoPonto#),999)>
			<!--- Obter a Perc_GrauPonto --->
			<cfset Perc_GrauPonto = NumberFormat(((VLR_Fin_Ponto / rsPtoMax.TUP_PontuacaoMaxima) * 100),999.00)>
	
			<!--- calcular o perc de classificacao do item --->	
			<cfset PercClassifPonto = NumberFormat(((Perc_GrauPonto / rsPtoMax.TUP_PontuacaoMaxima) * 100),999)>	
			
			<!--- calculo da descricao do item a saber GRAVE, MEDIANO ou LEVE --->
		
			<cfif PercClassifPonto gt 50.01>
			<cfset ClassifPonto = 'GRAVE'> 
			<cfelseif PercClassifPonto gte 10 and PercClassifPonto lte 50.01>
			<cfset ClassifPonto = 'MEDIANO'> 
			<cfelseif PercClassifPonto lt 10>
			<cfset ClassifPonto = 'LEVE'> 
			</cfif>	
		<cfelse>	
		   <cfset ClassifPonto = ItnClassificacao>
		</cfif>
			
		<!--- Ajustes para os campos: Pos_ClassificacaoPonto --->
		<cfquery datasource="#dsn_inspecao#">
			UPDATE ParecerUnidade SET Pos_ClassificacaoPonto = '#ClassifPonto#' 
			WHERE Pos_Unidade='#unid#' AND Pos_Inspecao='#ninsp#' AND Pos_NumGrupo=#ngrup# AND Pos_NumItem=#nitem#
		</cfquery>		
 <!---  <cfoutput>ptomax:#rsPtoMax.TUP_PontuacaoMaxima# ano: #right(ninsp,4)# tipo unid: #sfrmTipoUnidade# fatorvlrfin:#Fator_VLR_Fin# VLR_Fin_Ponto: #VLR_Fin_Ponto# Perc_GrauPonto: #Perc_GrauPonto# ClassifPonto: #ClassifPonto#</cfoutput>
		<cfset gil = gil> --->  		
    </cfif>	
 </cfif>	 --->	
</cfif>
<!--- Excluir Processo --->
 <cfif Form.acao is 'Excluir_Proc'>
	...
	<cfset pos_aux = Form.H_obs & CHR(13) & CHR(13) & DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '> Registro de Processo Disciplinar(Exclusão)' & CHR(13) & CHR(13) & 'Data de Atualização: ' & #DateFormat(now(),"DD/MM/YYYY")# & CHR(13) & CHR(13) & 'Número do Processo: ' & #aux_proc# & CHR(13) & CHR(13) & 'Modalidade: ' & #form.frmpdc_procmodal# & CHR(13) & CHR(13) & 'Nº SEI(Processo): ' & #aux_sei# & CHR(13) & CHR(13) &  'Responsável: ' & #CGI.REMOTE_USER# & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '-----------------------------------------------------------------------------------------------------------------------'>
    <cfquery datasource="#dsn_inspecao#">
     UPDATE ParecerUnidade SET Pos_Parecer= '#pos_aux#' WHERE Pos_Unidade='#unid#' AND Pos_Inspecao='#ninsp#' AND Pos_NumGrupo=#ngrup# AND Pos_NumItem=#nitem#
    </cfquery>	
    <cfquery name="rsSitAtual" datasource="#dsn_inspecao#">
...
<cfif Form.acao is "Incluir_Proc">
	...
	<cfelse>
		...
		<cfif qExitesSEI.RecordCount eq 0>
			...
			<cfset pos_aux = Form.H_obs & CHR(13) & CHR(13) & DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '> Registro de Nº SEI Apuração(Inclusão)' & CHR(13) & CHR(13) & 'Data de Atualização: ' & #DateFormat(now(),"DD/MM/YYYY")# & CHR(13) & CHR(13) & 'Número do SEI: ' & #FORM.frmprocsei# & CHR(13) & CHR(13) &  'Responsável: ' & #CGI.REMOTE_USER# & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '-----------------------------------------------------------------------------------------------------------------------'>
			<cfset and_aux = DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '> Registro de Nº SEI Apuração(Inclusão)' & CHR(13) & CHR(13) & 'Data de Atualização: ' & #DateFormat(now(),"DD/MM/YYYY")# & CHR(13) & CHR(13) & 'Número do SEI: ' & #FORM.frmprocsei# & CHR(13) & CHR(13) &  'Responsável: ' & #CGI.REMOTE_USER# & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '-----------------------------------------------------------------------------------------------------------------------'>
		</cfif>
	</cfif>	
 ...
 <cfif Form.acao is 'Excluir_Sei'>
    ...
    <cfset pos_aux = Form.H_obs & CHR(13) & CHR(13) & DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '> Registro de Nº SEI Apuração(Exclusão)' & CHR(13) & CHR(13) & 'Data de Atualização: ' & #DateFormat(now(),"DD/MM/YYYY")# & CHR(13) & CHR(13) & 'Número do SEI: ' & #aux_sei# & CHR(13) & CHR(13) &  'Responsável: ' & #CGI.REMOTE_USER# & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '-----------------------------------------------------------------------------------------------------------------------'>
    <cfquery datasource="#dsn_inspecao#">
     UPDATE ParecerUnidade SET Pos_Parecer= '#pos_aux#' WHERE Pos_Unidade='#unid#' AND Pos_Inspecao='#ninsp#' AND Pos_NumGrupo=#ngrup# AND Pos_NumItem=#nitem#
    </cfquery>	
    <cfquery name="rsSitAtual" datasource="#dsn_inspecao#">
	...
	<cfif rsAnd.recordcount gt 0>
		<cfset and_aux = #rsAnd.and_Parecer# & CHR(13) & CHR(13) & DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '> Registro de Nº SEI Apuração(Exclusão)' & CHR(13) & CHR(13) & 'Data de Atualização: ' & #DateFormat(now(),"DD/MM/YYYY")# & CHR(13) & CHR(13) & 'Número do SEI: ' & #FORM.dbfrmnumsei# & CHR(13) & CHR(13) &  'Responsável: ' & #CGI.REMOTE_USER# & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '-----------------------------------------------------------------------------------------------------------------------'> 
		<cfquery datasource="#dsn_inspecao#">
			UPDATE Andamento SET and_Parecer= '#and_aux#' 
			WHERE And_Unidade='#unid#' AND And_NumInspecao='#ninsp#' AND And_NumGrupo=#ngrup# AND And_NumItem=#nitem# and And_DtPosic = #prazo# and And_Situacao_Resp = #rsSitAtual.Pos_Situacao_Resp#
		</cfquery>
...
<cfif Form.acao is 'Anexar'>
	<cftry>
		...
		<!--- O arquivo anexo recebe nome indicando Numero da Avaliação, NÃºmero da unidade, NÃºmero do grupo e NÃºmero do item ao qual estão vinculado --->
...
<cfif isDefined("Form.Ninsp") and form.Ninsp neq "">
	...
	<cfparam name="URL.DtInic" default="#dateformat(form.dtinicio,"dd/mm/yyyy")#">
	<cfparam name="URL.dtFim" default="#dateformat(form.dtfinal,"dd/mm/yyyy")#">
			...
			<cfquery datasource="#dsn_inspecao#">
		 		...
		 		<cfset IDArea = #form.unid#>
		  		<cfswitch expression="#Form.frmResp#">
					...
					<!--- Status: Tratamento pela SE --->
					  ...
					  <cfset Gestor = '#rsMod.Dir_Descricao#'>
					  <cfset IDArea = #rsMod.Dir_Sto#>
					  <cfset situacao = 'TRATAMENTO SUPERINTENDÊNCIA ESTADUAL'>
					  <cfset sdestina = #rsMod.Dir_Email#>
					  <cfset nomedestino = #rsMod.Dir_Descricao#>
					</cfcase>
					<cfcase value=24>
				...
				<!--- 	   <cfquery name="qArea2" datasource="#dsn_inspecao#">
				 ...
				 <cfif IsDefined("FORM.nci") AND FORM.nci EQ "Sim">
				  ...
				  , Pos_Parecer=
				  <!--- As linhas abaixo servem para substituir o uso de aspas simples e duplas, a fim de evitar erros em instruÃ§Ãµes SQL --->
					  <cfset aux_obs = Trim(FORM.observacao)>
					  <cfset aux_obs = Replace(aux_obs,'"','','All')>
					  <cfset aux_obs = Replace(aux_obs,"'","","All")>
					  <cfset aux_obs = Replace(aux_obs,'*','','All')>
					...
					<!---	 <cfset aux_obs = Replace(aux_obs,'&','','All')>
					 ...
					 <cfif "#Form.frmResp#" eq 25 || "#Form.frmResp#" eq 26 >
						<cfset pos_aux = Form.H_obs & CHR(13) & CHR(13) & DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '> ' & #Trim(Encaminhamento)# & CHR(13) & CHR(13) & 'A(O)' & '  ' & #Gestor# & CHR(13) & CHR(13) & #aux_obs# & CHR(13) & CHR(13) & CHR(13) & CHR(13) & 'Situação: ' & #situacao# & CHR(13) & CHR(13) &  'Responsável: ' & #CGI.REMOTE_USER# & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '-----------------------------------------------------------------------------------------------------------------------'> 
					 </cfif> 	 
...
<!---  --->
 				...
 				<cfquery datasource="#dsn_inspecao#">
				  ...
				  <cfif IsDefined("FORM.observacao") AND FORM.observacao NEQ "">
					...
					<cfif "#Form.frmResp#" eq 25 || "#Form.frmResp#" eq 26 >
						<cfset and_obs = DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '> ' & #Trim(Encaminhamento)#  & CHR(13) & CHR(13) & 'AO (À) ' & '  ' & #Gestor# & CHR(13) & CHR(13) & #aux_obs# & CHR(13) & CHR(13) & CHR(13) & CHR(13) & 'Situação: ' & #situacao# & CHR(13) & CHR(13) &  'Responsável: ' & #CGI.REMOTE_USER# & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '-----------------------------------------------------------------------------------------------------------------------'>
					</cfif>	 	 
				  '#and_obs#'
				  <cfelse>
				   NULL
					...
					<cfif Form.frmResp is 15 or Form.frmResp is 16 or Form.frmResp is 18 or Form.frmResp is 19 or Form.frmResp is 23>
						  <cfoutput>
								  <cfif Form.frmResp is 15 or Form.frmResp is 18>
										<!--- Participar ao Orgï¿½o subordinador --->
								...
								&nbsp;&nbsp;&nbsp;O registro da manifestação estão disponível no link: <a href="http://intranetsistemaspe/snci/rotinas_inspecao.cfm">Relat&oacute;rio Item Em Tratamento.</a><br><br>
...
INNER JOIN Grupos_Verificacao ON convert(char(4), RIP_Ano) = Itn_Ano and (Itn_Modalidade = INP_Modalidade) and (Itn_Ano = Grp_Ano) AND (Itn_NumGrupo = Grp_Codigo) and (Itn_TipoUnidade = Und_TipoUnidade)) ON Dir_Codigo = Und_CodDiretoria
...
--->
		  ...
		  <cfif #PrazoVencido# eq 'yes'>
			  <cfif Trim(rsUsuarioLogado.GrupoAcesso) eq 'GESTORMASTER'>
			   AND STO_Codigo in (9,12,13,21,25,30,31)
			  <cfelse>  
			   AND STO_Codigo in (9,12,13,18,25,30,31)	
			  </cfif>
		  ...
		  <cfelse>
			  <cfif Trim(rsUsuarioLogado.GrupoAcesso) eq 'GESTORMASTER'>
			   AND STO_Codigo in (9,12,13,21,25,30,31)
			  <cfelse>
			   AND STO_Codigo in (9,12,13,25,30,31)   
			  </cfif>
	      </cfif>
		  order by STO_Descricao
		</cfquery>
...
<cfelse>
	...
	<cfquery name="rsPonto" datasource="#dsn_inspecao#">
		<cfif qResposta.Pos_Situacao_Resp neq 9>
		   SELECT STO_Codigo, STO_Descricao FROM Situacao_Ponto WHERE STO_Status='A' AND STO_Codigo not in (1,2,4,5,6,7,8,11,14,17,18,20,22,25,27) 
	    <cfelse>
           SELECT STO_Codigo, STO_Descricao FROM Situacao_Ponto WHERE STO_Status='A' AND STO_Codigo not in (1,2,4,5,6,7,8,9,11,14,17,20,22,25,27) 			
	    </cfif>
...
//alert('acao: ' + document.form1.acao.value + ' resposta:' + document.form1.frmResp.value + ' tamanho: ' + tamresp.length);
  ...
  //=============================================================
	if (document.form1.acao.value=='alter_reincidencia'){
			...
			{
			 alert('Para o Reincidência: Sim é preciso informar Nº Avaliação, Nº Grupo e Nº Item!');
			 //exibe(sit);
			 return false;
			} else {
			document.form1.frmreincInsp.disabled = false;
...
if (document.form1.acao.value=='Incluir_Proc'){
 ...
 //===============================
  // inicio cri­ticas para o botão Salvar manifestação
...
//==================================
	  if (sit == 9)
	  {
		...
		{
		   alert('Sr. Gestor, informe a Área(CS) para encaminhamento!');
		  // exibe(sit);
		   return false;
		}
	  }
...
//-----------------------------------------------------------
	<!--- formato AAAAMMDD --->
		...
		//alert('Qual o valor da situacao? ' + sit);
		 ...
		 {
		  alert("Para Tratamento a Data de Previsão não deve ser superior aos 365 dias!")
		  //dtprazo(sit);
		  exibe(sit);
		  return false;
		 }		
...
var page = "itens_analistas_controle_respostas_comentarios.cfm?numero=#ninsp#&unidade=#unid#&numgrupo=#ngrup#&numitem=#nitem#";
</cfoutput>
...
<body onLoad="exibevalores(); if(document.form1.houveProcSN.value != 'S') {exibe(document.form1.frmResp.value)} else {exibe(24)}; reincidencia(document.form1.frmReinccb.value); hanci(); controleNCI(); exibir_Area011(this.value)"> 
<cfset form.acao = ''>
...
<table width="85%"  align="center" bordercolor="f7f7f7">
  ...
  <form name="form1" method="post" onSubmit="return validarform()" enctype="multipart/form-data" action="itens_analistas_controle_respostas1.cfm">
  <cfoutput>
    ...
    <tr>
      <td colspan="5"><p class="titulo1">
        ...
        <input name="dtinicio" type="hidden" id="dtinicio" value="<cfoutput>#dateformat(URL.DtInic,"dd/mm/yyyy")#</cfoutput>">
        <input name="dtfinal" type="hidden" id="dtfinal" value="<cfoutput>#dateformat(URL.dtfim,"dd/mm/yyyy")#</cfoutput>">
...
<!--- 	  <cfset habslvsn = 'S'>
	  <cfif (left(unid,2) neq left(posarea,2)) and (qUsuario.Usu_DR neq left(url.PosArea,2))>
	    <cfset habslvsn = 'N'>
	  </cfif>  ---> 
    </tr>
	  <tr>
   ...
   <tr class="exibir">
    ...
    <tr class="exibir">
      ...
      <td colspan="4" bgcolor="f7f7f7"><cfoutput><strong class="exibir">#URL.Ninsp#</strong></cfoutput>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;In&iacute;cio 
...
Avalia&ccedil;&atilde;o &nbsp;<strong class="exibir"><cfoutput>#DateFormat(qResposta.INP_DtInicInspecao,"dd/mm/yyyy")#</cfoutput></strong></td>
    ...
    <tr class="exibir">
      ...
      <td colspan="4" bgcolor="f7f7f7"><cfoutput><strong class="exibir">#URL.Ngrup#</strong></cfoutput>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<cfoutput><strong class="exibir">#qResposta.Grp_Descricao#</strong></cfoutput></td>
    </tr>
...
<!--- 	<tr class="exibir">
	  <td bgcolor="f7f7f7">Pontua&ccedil;&atilde;o Item: </td>
	  <td bgcolor="f7f7f7"><strong class="exibir"><cfoutput>#riponto#</cfoutput></strong></td>
	  <td bgcolor="f7f7f7"><div align="right">Classifica&ccedil;&atilde;o Item:</div></td>
	  <td bgcolor="f7f7f7"><div align="left"><strong class="exibir"><cfoutput>#auxclass#</cfoutput></strong></div></td>
...
<tr class="exibir">
	  ...
	  <td colspan="4" bgcolor="f7f7f7"><table width="100%" border="0">
        <tr class="exibir">
          ...
          <td width="237"><strong class="exibir">#qResposta.Pos_PontuacaoPonto#</strong></td>
          <td width="169"><div align="right">Classificação do Ponto &nbsp;</div></td>
          <td width="538"><strong class="exibir">#qResposta.Pos_ClassificacaoPonto#</strong></td>
        </tr>
      </table></td>
	  </tr>
	<tr class="exibir">
	  <td bgcolor="f7f7f7">Reincidência</td>
	  <cfif trim(qResposta.RIP_ReincInspecao) eq "" || trim(qResposta.RIP_ReincInspecao) eq "0"><!---Foi colocado zero pois o xml gerado pelo SGI está colocando 0(zero) por padrão--->
...
<input name="frmreincItem" type="text" class="form" id="frmreincItem" size="7" maxlength="4" value="#db_reincItem#" onKeyPress="numericos(this.value)">
</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input name="alter_reincidencia" id="alter_reincidencia" type="submit" class="botao" onClick="CKupdate();document.form1.acao.value='alter_reincidencia'; document.form1.Salvar2.disabled = true; document.form1.frmResp.disabled = true" value="Confirmar" disabled>
</td>
	</tr>
	...
	<cfoutput>
      ...
      <td colspan="4" bgcolor="f7f7f7">
		  ...
		  <tr class="form">
					...
					<input type="hidden" name="PosPontuacaoPonto" id="PosPontuacaoPonto" value="#qResposta.Pos_PontuacaoPonto#">
					<input type="hidden" name="ItnClassificacao" id="ItnClassificacao" value="#qResposta.Itn_Classificacao#">
	 ...
	 <cfset melhoria = '#qResposta.RIP_Comentario#'>
        ...
        <td colspan="5" bgcolor="f7f7f7"><textarea name="Melhoria" cols="200" rows="20" wrap="VIRTUAL" class="form" readonly><cfoutput>#melhoria#</cfoutput></textarea></td>
      </tr>
 ...
 <!---  </cfif> --->
    ...
    <tr>
      <td valign="middle" bgcolor="eeeeee" align="center"><p><span class="titulos">Hist&oacute;rico:</span> <span class="titulos">Manifestações e Plano de Ação/Análise do Controle Interno</span><span class="titulos">:</span></p>
        <p>
    ...
    <tr bgcolor="f7f7f7">
      ...
      <td colspan="5">
        ...
        <tr>
			<td colspan="3" bgcolor="eeeeee"><span class="exibir">Nº SEI:</span></td>
			<td bgcolor="eeeeee"><div align="left">
			<input name="frmprocsei" id="frmprocsei" type="text" class="form" onKeyPress="numericos()" onKeyDown="validacao(); Mascara_SEI(this)" size="27" maxlength="20" value="">
			</td>
			<td bgcolor="eeeeee" class="exibir">	  
				  <span class="exibir">Nº GPAC:</span>&nbsp;&nbsp;
			...
			<td colspan="2" bgcolor="eeeeee">
				<cfif (resp neq 24)>
					<input name="btn_incluirProc" id="btn_incluirProc" type="submit" class="botao" onClick="document.form1.acao.value='Incluir_Proc';frmprocsei.value" value="Incluir-Proc. Disciplinar" codigo="#frmprocsei.value#">
				<cfelse>
	...
	<cfoutput query="qSeiApur">
          ...
          <tr bgcolor="f7f7f7">
			...
			<cfif (resp neq 24) and (qSeiApur.recordcount gt 0)>
				<td bgcolor="eeeeee"><input name="btn_ExcluirSei" id="btn_ExcluirSei" type="submit" class="botao" onClick="document.form1.acao.value='Excluir_Sei';document.form1.dbfrmnumsei.value='#qSeiApur.SEI_NumSEI#'" value="Excluir Nº SEI" codigo="#qSeiApur.SEI_NumSEI#"></td>
...
<!--- FIM - Direcionar ponto para APURACAO  --->		  
    ...
    <tr bgcolor="f7f7f7">
      ...
      <td bgcolor="eeeeee">
		<cfif (resp neq 24)>
		  <input name="btn_inc_causa" id="btn_inc_causa" type="Submit" class="botao" value="Incluir Causa" onClick="document.form1.acao.value='Incluir_Causa';"></td>
		<cfelse>
		  <input name="btn_inc_causa" id="btn_inc_causa" type="Submit" class="botao" value="Incluir Causa" onClick="document.form1.acao.value='Incluir_Causa';" disabled></td>
		</cfif>
    </tr>
    ...
    <cfif isDefined("Session.E01.abertura")>
      <tr bgcolor="f7f7f7">
        <td height="22" colspan="5" valign="middle" bgcolor="eeeeee" class="exibir">Situação:
	   ...
	   <input name="VLRecuperado" type="text" class="form"  size="22" maxlength="16"  onFocus="moeda_dig(this.name)" onKeyPress="press_tecla(this.name)" onKeyUp="soltar_tecla(this.name)"  onBlur="ajuste_campo(this.name)">
	  </td>
        </tr>
        <tr bgcolor="f7f7f7">
...
<tr bgcolor="eeeeee">
	   ...
	   <td colspan="5" bgcolor="eeeeee" class="exibir">Nº Processo Judicial:&nbsp;&nbsp;
	     <input name="posnumprocjudicial" type="text" class="form" value="<cfoutput>#npjudic#</cfoutput>"  size="40" maxlength="30">
	  </td>
   ...
   <cfelse>
     <tr>
      <td colspan="5">
		...
		<table width="100%" border="0">
			<tr bgcolor="f7f7f7">
				<td colspan="5" valign="middle" bgcolor="eeeeee" class="exibir">Houve Nota de Controle?&nbsp;&nbsp;<input name="nci" id="nci" type="text" class="form" size="4" maxlength="3" value="<cfoutput>#auxnci#</cfoutput>" readonly>
				</td>
				<cfif auxnci eq "Sim">
					<td width="77%" bgcolor="eeeeee" class="exibir">Nº SEI da NCI :&nbsp;&nbsp;
					   <input name="frmnumseinci" id="frmnumseinci" type="text" class="form" onKeyPress="numericos()" onKeyDown="validacao(); Mascara_SEI(this)" size="27" maxlength="20" value="<cfoutput>#numncisei#</cfoutput>" ReadOnly>
					</td>
				</cfif>
			</tr>
        </table>
...
<tr bgcolor="eeeeee">
        <td height="22" colspan="3" bgcolor="eeeeee" class="exibir"><div id="dArea">Selecione a Àrea:
	...
	<cfelse>
	     <cfset vlrrecaux = #mid(LSCurrencyFormat(qResposta.Pos_VLRecuperado, "local"), 4, 20)#>
		   ...
		   <td colspan="2" bgcolor="eeeeee" class="exibir"><div id="dValor">Valor Regularizado(R$):&nbsp;&nbsp;
		   <input name="VLRecuperado" type="text" class="form"  size="22" maxlength="17"  onFocus="moeda_dig(this.name)" onKeyPress="moeda_dig(this.name)" onKeyUp="moeda_edit(this.name)"  onBlur="ajuste_campo(this.name)">
		  </td>
      </tr>

      <tr bgcolor="eeeeee">
        <td colspan="5" class="exibir"><div id="dAreaCS">Selecione a Àrea:
            <select name="cbareaCS" class="form">
              <option selected="selected" value="">---</option>
              <cfoutput query="qAreaCS">
				<option value="#Ars_Codigo#" <cfif #Ars_Codigo# is #qResposta.Pos_Area#>selected</cfif>>#trim(Ars_Descricao)#</option>
...
<!--- 	   itens_analistas_controle_respostas.cfm?ninsp=#URL.Ninsp#&unid=#URL.Unid#&ngrup=#URL.Ngrup#&nitem=#URL.Nitem#&dtfim=#url.dtfim#&dtinic=#url.dtinic#&SE=#url.SE#&cktipo=#url.cktipo#&selstatus=#selstatus#&StatusSE=#StatusSE# --->
	   	 <input name="button" type="button" class="botao" onClick="window.open('itens_analistas_controle_respostas.cfm?ninsp=#URL.Ninsp#&unid=#URL.Unid#&ngrup=#URL.Ngrup#&nitem=#URL.Nitem#&dtfim=#url.dtfim#&dtinic=#url.dtinic#&SE=#url.SE#&cktipo=#url.cktipo#&selstatus=#selstatus#&StatusSE=#StatusSE#','_self')" value="Voltar">
       <cfelse>
...
<!--- 	  	<cfif ((resp is 1 and numdias lte 2) or (resp is 6 and numdias lte 2) or (resp is 7 and numdias lte 2) or (resp is 17 and numdias lte 2) or (resp is 22 and numdias lte 2) or (resp is 18) or (resp is 24))>
	  ...
	  <cfelse>
        <input name="Salvar2" type="submit" class="botao" value="Salvar" onClick="CKupdate();document.form1.acao.value='Salvar2'">
	  	  	...
	  	  	<cfif ((resp is 10) or (resp is 18) or (resp is 24) or (resp is 31))>
		<input name="Salvar2" type="submit" class="botao" value="Salvar" onClick="CKupdate();document.form1.acao.value='Salvar2';" disabled>
	  <cfelse>
        <input name="Salvar2" type="submit" class="botao" value="Salvar" onClick="CKupdate();document.form1.acao.value='Salvar2'">
      </div></cfif>
...
<!--- 	
	<input type="hidden" name="tuiPontuacao" id="tuiPontuacao" value="<cfoutput>#qResposta.tui_Pontuacao#</cfoutput>"> --->
...
<script>
	<cfoutput>
		<!---Retorna true se a data de início da Avaliação for maior ou igual a 04/03/2021, data em que o editor de texto foi implantado. Isso evitará que os textos anteriores sejam desformatados--->
