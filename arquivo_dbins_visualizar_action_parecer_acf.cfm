...
<cffile action="read" file="#servidor_ftp##FORM.nome_arquivo#" variable="XmlDoc">
...
<cfif qVerificaParecer.recordCount is 1>	
   <cfset aux_Parecer = Trim(xml.rootelement.dados[i].Pos_Parecer.XmlText)>
   <cfset aux_Parecer = Replace(aux_Parecer,'"','','All')>
   <cfset aux_Parecer = Replace(aux_Parecer,"'","","All")>
   <cfset aux_Parecer = Replace(aux_Parecer,'&','','All')>
   <cfset aux_Parecer = Replace(aux_Parecer,'*','','All')>
<!--- <cfset aux_Parecer = Replace(aux_Parecer,'%','','All')> --->
...
SET 	Pos_DtPosic = #xml.rootelement.dados[i].Pos_DtPosic.XmlText# ,
		...
		Pos_Situacao = 'AN', <!--- '#xml.rootelement.dados[i].Pos_Situacao.XmlText#', --->
		Pos_Parecer = '#aux_Parecer#',
		Pos_Situacao_Resp = '1', <!--- '#xml.rootelement.dados[i].Pos_Situacao_Resp.XmlText#' --->
...
<table width="780">
	<tr>
		<td colspan="2" class="titulos"><div align="center">Respostas das ACF's</div></td>
	</tr>
	<tr>
...
<table width="780">
	...
	<tr>
	  <!--- Esta mensagem aparece caso o arquivo de inspeção não seja transmitido. --->
