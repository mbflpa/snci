...
WHERE (And_DtPosic BETWEEN #url.dtInicio# AND #url.dtFinal#) and (trim(Usu_GrupoAcesso)= 'INSPETORES' Or trim(Usu_GrupoAcesso) = 'GESTORES' Or trim(Usu_GrupoAcesso) = 'ANALISTAS') and (And_Situacao_Resp in (3,10,12,13,15,16,18,19,23,24,25,26,28,29,30,31))
<cfif URL.SE gt 0>
...
function valida_form() {
  ...
  if (frm.ckTipo.value=='super'){
   ...
   if (frm.ckTipo.value=='inspecao'){
		...
		alert("Nº do Relatório deve conter 10 dígitos!");
		frm.txtNum_Inspecao.focus();
		return false;
	    }
	}
...
<!--- �rea de conte�do   --->
	<form action="Itens_Analise_Manifestacao1.cfm" method="get" target="_blank" name="frmObjeto" onSubmit="return valida_form()">
      <table width="46%" align="center">
        <tr>
          <td colspan="4" align="center" class="titulo1">ANÁLISES  DAS MANIFESTAÇÕES</td>
        </tr>
        <tr>
