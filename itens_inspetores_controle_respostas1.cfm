...
<!--- Excluir Processo --->
 <cfif isDefined("Form.acao") and Form.acao is 'Excluir_Proc'>
...
<cfif isDefined("Form.acao") and Form.acao is "Incluir_Proc">
	...
	<cfelse>
		...
		<cfif qExitesSEI.RecordCount eq 0>
			...
			<cfset pos_aux = Form.H_obs & CHR(13) & CHR(13) & DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '> Registro de N.SEI Apuração(Inclusão)' & CHR(13) & CHR(13) & 'Data de Atualização: ' & #DateFormat(now(),"DD/MM/YYYY")# & CHR(13) & CHR(13) & 'Número do SEI: ' & #FORM.frmprocsei# & CHR(13) & CHR(13) &  'Responsável: ' & #maskcgiusu# & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '-----------------------------------------------------------------------------------------------------------------------'>
 ...
 <!--- Excluir N. SEI --->
 <cfif isDefined("Form.acao") and Form.acao is 'Excluir_Sei'>
	<cfquery datasource="#dsn_inspecao#">
	...
	<cfif rsAnd.recordcount gt 0>
		<cfset and_aux = #rsAnd.and_Parecer# & CHR(13) & CHR(13) & DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '> Registro de N.SEI Apuração(Exclusão)' & CHR(13) & CHR(13) & 'Data de Atualização: ' & #DateFormat(now(),"DD/MM/YYYY")# & CHR(13) & CHR(13) & 'Número do SEI: ' & #FORM.dbfrmnumsei# & CHR(13) & CHR(13) &  'Responsável: ' & #maskcgiusu# & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '-----------------------------------------------------------------------------------------------------------------------'> 
		<cfquery datasource="#dsn_inspecao#">
			UPDATE Andamento SET and_Parecer= '#and_aux#' 
			WHERE And_Unidade='#unid#' AND And_NumInspecao='#ninsp#' AND And_NumGrupo=#ngrup# AND And_NumItem=#nitem# and And_DtPosic = #prazo# and And_Situacao_Resp = #rsSitAtual.Pos_Situacao_Resp#
		</cfquery>
...
<cfif isDefined("Form.acao") and Form.acao is "Incluir_Causa" and Form.causaprovavel neq "">
...
<!--- <cfoutput>
  ...
  <!--- Excluir Causa --->
	<cfif isDefined("Form.acao") and Form.acao is 'Excluir_Causa'>
...
not in()
  ...
  <!--- As linhas abaixo servem para substituir o uso de aspas simples e duplas, a fim de evitar erros em instrua§aµes SQL --->
	...
	<!---	 <cfset aux_obs = Replace(aux_obs,'&','','All')>
        ...
        <cfset pos_aux = Form.H_obs & CHR(13) & CHR(13) & DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '> ' & #Trim(Encaminhamento)# & CHR(13) & CHR(13) & 'A(O)' & '  ' & #Gestor# & CHR(13) & CHR(13) & #aux_obs# & CHR(13) & CHR(13) & 'Data de Previsao da Solução: ' & #DateFormat(dtnovoprazo,"DD/MM/YYYY")# & CHR(13) & CHR(13) & 'Situação: ' & #situacao# & CHR(13) & CHR(13) &  'Responsável: ' & #maskcgiusu# & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '-----------------------------------------------------------------------------------------------------------------------'>
...
<cfif Form.frmResp is 15 or Form.frmResp is 16 or Form.frmResp is 18 or Form.frmResp is 19 or Form.frmResp is 23>
	...
	&nbsp;&nbsp;&nbsp;Comunicamos que ha pontos de Controle Interno "Em Tratamento" de manifestacao/Solucao.<br><br>

	&nbsp;&nbsp;&nbsp;Unidade: #FORM.unid# - #rsMod.Und_Descricao#, Avaliacao: #form.ninsp#, Grupo: #form.Ngrup#, Item: #form.Nitem# e Data de Previsao Solucao: #DateFormat(dtnovoprazo,"DD/MM/YYYY")#.<br><br>

	&nbsp;&nbsp;&nbsp;O registro da manifestacao estao disponível no link: <a href="http://intranetsistemaspe/snci/rotinas_inspecao.cfm">Relatorio Item Em Tratamento.</a><br><br>

	&nbsp;&nbsp;&nbsp;Desde ja agradecemos a sua atencao.
	</strong>
	</cfmail>
	</cfoutput>
	</cfif>
	...
	<cfoutput query="qSeiApur">
          ...
          <tr bgcolor="eeeeee">
			...
			<cfif (resp neq 24) and (qSeiApur.recordcount gt 0)>
				<td bgcolor="eeeeee"><input name="btn_ExcluirSei" id="btn_ExcluirSei" type="submit" class="botao" onClick="document.form1.acao.value='Excluir_Sei';document.form1.dbfrmnumsei.value='#qSeiApur.SEI_NumSEI#'" value="Excluir N. SEI" codigo="#qSeiApur.SEI_NumSEI#"></td>
...
<!--- 		<cfif ((resp is 1 and numdias lte 2) or (resp is 6 and numdias lte 2) or (resp is 7 and numdias lte 2) or (resp is 17 and numdias lte 2) or (resp is 18) or (resp is 24) or (#qUsuario.Usu_DR# neq #left(url.PosArea,2)#))> --->
		...
		<cfelse>
		  <input name="btn_inc_causa" id="btn_inc_causa" type="Submit" class="botao" value="Incluir Causa" onClick="document.form1.acao.value='Incluir_Causa';">
...
<!---    <input name="button" type="button" class="botao" onClick="window.open('itens_controle_respostas.cfm?ckTipo=inspecao&txtNum_Inspecao=#URL.Ninsp#&Submit2=Confirmar','_self')" value="Voltar"> --->
	   ...
	   <cfif houveProcSN neq "S">
	     <input name="button" type="button" class="botao" onClick="window.open('itens_inspetores_controle_respostas.cfm?ninsp=#URL.Ninsp#&unid=#URL.Unid#&ngrup=#URL.Ngrup#&nitem=#URL.Nitem#&dtfim=#auxdtfim#&dtinic=#auxdtini#&SE=#url.SE#&cktipo=#url.cktipo#&selstatus=#selstatus#&StatusSE=#StatusSE#','_self')" value="Voltar">
       <cfelse>
...
<!---    	  <cfif ((resp is 1 and numdias lte 2) or (resp is 6 and numdias lte 2) or (resp is 7 and numdias lte 2) or (resp is 17 and numdias lte 2) or (resp is 22 and numdias lte 2) or (resp is 18) or (resp is 24) or (habslvsn eq 'N'))>
	 ...
	 <cfif ((resp is 18) or (resp is 24)  or (resp is 31) or (habslvsn eq 'N'))> 
	 		<input name="Salvar2" type="submit" class="botao" value="Salvar" onClick="CKupdate();document.form1.acao.value='Salvar2';" disabled>
	  <cfelse>
        <input name="Salvar2" type="submit" class="botao" value="Salvar" onClick="CKupdate();document.form1.acao.value='Salvar2'">
      </cfif>
