...
DELETE FROM Resultado_Inspecao WHERE RIP_NumInspecao = '1621212021'
</cfquery>
<cfquery datasource="#dsn_inspecao#">
DELETE FROM Inspecao WHERE INP_NumInspecao = '1621212021'
</cfquery>
<cfquery datasource="#dsn_inspecao#">
DELETE FROM Numera_Inspecao WHERE NIP_NumInspecao = '1621212021'
</cfquery>
...
<cfdirectory name="qList" filter="*.xml" sort="name asc" directory="#slocal#">
...
<!---  --->
<!---  <cfoutput>
 destination: #GetDirectoryFromPath(GetTemplatePath())#Dados<br>
 form.arquivo: #Form.arquivo#<br>
 form.numero: #form.numero_inspecao#<br>
 nomearquivo: #right(form.numero_inspecao,14)#<br>
 form.numrelat: #form.numrelat#<br>
 arquivo: #arquivo#<br>
<!---  xml: #xml#<br> --->
 </cfoutput>   --->
<!---   <cfset gil = gil>	  --->
<cfif len(trim(nomearquivo)) neq 14>
	 <cfoutput>
...
<!---  xml: #xml#<br> --->
  ...
  <cfset gil = gil>	 --->
    ...
    <cfif right(RIPInspecao,4) neq RIPANO>
         <cfoutput>
              <cflocation url="arquivo_dbins_RejeitarXML.cfm?form.motivo=O Ano da Inspecao difere ao do exercicio realizado informado&form.numrelat=#RIPInspecao#&form.numunid=#RIPUnidade#&form.dtiniinsp=#INPDtInicInsp#&form.nomearquivo=#nomearquivo#&form.excluirsn=N">
         </cfoutput>
    </cfif>
...
<div align="center">
 ...
 <form name="frm1" action="monitoramento_recep_xml2.cfm?flush=true" method="post">
<table align="center">
<!---   <cfif isDefined("Form.arquivo")> --->
  <tr class="titulos">
  ...
  <table width="528" border="1" align="center" class="Tabela">
    <tr>
      <td colspan="2" class="titulosClaro2"><div align="center">AVALIAÇÃO DO CONTROLE INTERNO A SER MONITORADA ! </div></td>
    </tr>
    <tr>
      <td colspan="2">&nbsp;</td>
    </tr>
    ...
    <tr>
      <td class="titulos">Dt. In&iacute;cio da Inspeção</td>
