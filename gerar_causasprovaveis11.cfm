...
<cfquery name="rsItem" datasource="#dsn_inspecao#">
   ...
   INNER JOIN Grupos_Verificacao ON RIP_NumGrupo = Grp_Codigo AND right([RIP_NumInspecao], 4) = Grp_Ano ON Und_Codigo = INP_Unidade 
   INNER JOIN Itens_Verificacao ON Itn_NumGrupo = RIP_NumGrupo AND Itn_NumItem = RIP_NumItem  AND right([RIP_NumInspecao], 4) = Itn_Ano
...
<!--- 	<cfquery name="rsPonto" datasource="#dsn_inspecao#">
      SELECT STO_Codigo, STO_Descricao FROM Situacao_Ponto WHERE STO_Status='A' AND STO_Codigo not in (19,21)
	</cfquery> --->
<cfelse>
	<cfquery name="rsPonto" datasource="#dsn_inspecao#">
...
<!--- �rea de conte�do   --->
    <form name="form1" method="post" onSubmit="if (document.form1.acao.value == 'Salvar2') {return salvar(vPSitResp.value)}" enctype="multipart/form-data" action="itens_unidades_controle_respostas1_area.cfm">
      <div align="right"><table width="74%" align="center">
          <tr><br>
              <td colspan="9"><p align="center" class="titulo1"><strong> </strong><strong class="titulo1">consulta pendentes (departamento)</strong>
					  ...
					  <input name="Ano" type="hidden" id="Ano" value="<cfoutput>#URL.Ano#</cfoutput>">
          ...
          <tr>
            <td valign="middle" bgcolor="eeeeee" class="exibir"><div align="center"><span class="titulos">Situa&ccedil;&atilde;o Encontrada:</span></div></td>
		 ...
		 <tr>
            ...
            <td colspan="7"><span class="exibir"><textarea name="H_recom" cols="200" rows="12" wrap="VIRTUAL" class="form" readonly><cfoutput>#rsItem.RIP_Recomendacoes#</cfoutput></textarea></span></td>
		  </tr>
     ...
     <tr>
        <td bgcolor="eeeeee" class="exibir"><div align="center"><strong>Arquivo:</strong></div></td>
        <td colspan="7" bgcolor="eeeeee" class="exibir"><input name="arquivo" class="botao" type="file" size="50"></td>
     </tr>
      <tr>
      ...
      <cfloop query= "qAnexos">
        <cfif FileExists(qAnexos.Ane_Caminho)>
          <tr>
            <td colspan="7" bgcolor="eeeeee" class="form"><cfoutput>#ListLast(qAnexos.Ane_Caminho,'\')#</cfoutput></td>
            <td align="center" bgcolor="eeeeee"><cfset arquivo = ListLast(qAnexos.Ane_Caminho,'\')>
                <input type="button" class="botao" name="Abrir" value="Abrir" onClick="window.open('abrir_pdf_act.cfm?arquivo=<cfoutput>#arquivo#</cfoutput>','_blank')" /><span class="exibir"></span></td>
		 </tr>
        </cfif>
      </cfloop>
         <tr>
		 ...
		 <tr>
          ...
          <tr><cfoutput>
            <td colspan="8">
			 <div align="center">
			   <input name="button" type="button" class="botao" onClick="window.open('gerar_causasprovaveis1.cfm?#URL.Ninsp#&unid=#URL.Unid#&ngrup=#URL.Ngrup#&nitem=#URL.Nitem#&dtFim=#url.dtfim#&DtInic=#url.dtinic#&SE=#url.SE#&cktipo=#url.cktipo#&selstatus=#selstatus#&Ano=#ano#&StatusSE=#StatusSE#','_self')" value="Voltar">
			   </div>
			 </td></cfoutput>
            </tr>
        </table>
