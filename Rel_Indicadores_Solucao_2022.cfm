...
<!--- POS_NOMEAREA NÃO PERMITIDOS  --->
 <cfquery name="AndtArea" datasource="#dsn_inspecao#">
	SELECT Andt_AnoExerc, Andt_TipoRel, Andt_Mes, Andt_Unid, Andt_Insp, Andt_Grp, Andt_Item, Andt_Resp, Andt_Area, Andt_PosClassificacaoPonto
	FROM Andamento_Temp 
	WHERE Andt_AnoExerc ='#year(dtlimit)#' AND Andt_TipoRel=2 AND Andt_Mes = #month(dtlimit)# 
</cfquery>
<cfloop query="AndtArea">
	...
	<cfif rsNegar.recordcount gt 0 or AndtPosClassificacaoPonto eq 'LEVE'>
<!--- 		<cfquery datasource="#dsn_inspecao#">
		   UPDATE Andamento_Temp SET Andt_Prazo = 'EX'
...
<cflocation url="Rel_Indicadores_Solucao1_2022.cfm?se=#se#&Submit1=Confirmar&dtlimit=#dateformat(dtlimit,"yyyy/mm/dd")#&anoexerc=#year(dtLimit)#"> 
</cfoutput>
