...
function valida_form() {
  ...
  // alert(frm.ckTipo.value);
       ...
       //  alert('dt inicio: ' + dtinic_yyyymmdd + '     dt fim: ' + dtfim_yyyymmdd);
		 ...
		 var auxnumdias = eval(dtfim_yyyymmdd - dtinic_yyyymmdd);
		 var auxnumdias = (auxnumdias - 70);
		 if (eval(auxnumdias) > 31)
		 {
		 //alert(eval(dtfim_yyyymmdd) + ' ' + eval(dtinic_yyyymmdd));
		  alert('Esta consulta está limitada a 31(dias) entre as duas datas!')
		  frm.dtfim.focus();
		  return false;
		 }
		if (frm.SE.value==''){
  ...
  if (frm.ckTipo.value=='super'){
		...
		if (frm.superAno.value < 2018 ){
		  alert('Não há Avaliação para ano inferior a 2018');
		  frm.superAno.focus();
		  return false;
		}
	}
   ...
   if (frm.ckTipo.value=='inspecao'){
		...
		alert("Nº da Avaliação deve conter 10 dígitos!");
		frm.txtNum_Inspecao.focus();
		return false;
	    }
	}
...
<!--- �rea de conte�do   --->
	<form action="Itens_Analise_Manifestacao.cfm" method="get" target="_blank" name="frmObjeto" onSubmit="return valida_form()">
      <table width="59%" align="center">
        <tr>
          <td colspan="5" align="center" class="titulo1">ANÁLISES  DAS MANIFESTAÇÕES</td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td width="21%"><strong><span class="exibir">Data Inicial :</span></strong><strong></strong></td>
          <td colspan="2"><strong class="titulo1">
