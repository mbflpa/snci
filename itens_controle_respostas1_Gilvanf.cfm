...
<!---  <cftry>  --->
  ...
  <cfif isDefined("Form.frmnumsei")><cfset Session.E01.frmnumsei = Form.frmnumsei><cfelse><cfset Session.E01.frmnumsei = ''></cfif>
...
<!--- <cfdump var="#qUnidade#"> --->
 ...
 <cfif IsDefined("FORM.submit") AND IsDefined("FORM.acao") And Form.acao is "Salvar">
	 ...
	 <!--- Verificar registro está na situação 11(Em Liberação) na tabela ParecerUnidade --->
			 ...
			 <cfif qEmLiberacao.Pos_Situacao_Resp eq 11>
				 ...
				 <cfquery datasource="#dsn_inspecao#">
				   ...
				   VALUES ('#Form.ninsp#', '#Form.unid#', #Form.ngrup#, #Form.nitem#, convert(char, getdate(), 102), '#CGI.REMOTE_USER#', 11, convert(char, getdate(), 108),'#unid#')
				 </cfquery>
	        ...
	        <cfif qInspecaoLiberada.recordCount Eq 0>
				...
				<cfquery name="rs11" datasource="#dsn_inspecao#">
			     SELECT Und_TipoUnidade, Und_Centraliza, Itn_TipoUnidade, Pos_Unidade, Pos_NumGrupo, Pos_NumItem, Pos_Area, Pos_NomeArea FROM Itens_Verificacao INNER JOIN (ParecerUnidade INNER JOIN Unidades ON Pos_Unidade = Und_Codigo) ON (Itn_NumItem = Pos_NumItem) AND (Itn_NumGrupo = Pos_NumGrupo)
				...
				<cfloop query="rs11">
					...
					<cfif (trim(rs11.Und_Centraliza) neq "") and (rs11.Itn_TipoUnidade eq 4)>
		        ...
		        <cfoutput>
					...
					<cfif rsEmail.recordcount gt 0>
                        ...
                        <cfif emailreopcdd neq "">
						 ...
						 <cfif left(rsEmail.Pos_Unidade,2) is '20'>
						   <cfset sdestina = #sdestina# & ';fsobrinho@correios.com.br'>
						 </cfif>

						 <cfset assunto = 'Relatório de Controle Interno - ' & #trim(rsEmail.Und_Descricao)# & ' - INSPEÇÃO ' & #ninsp#>
						<cfmail from="SNCI@correios.com.br" to="#sdestina#" subject="#assunto#" type="HTML">
							Mensagem automática. Não precisa responder!<br><br>
							<strong>
							   Prezado(a) Gerente do(a) #trim(Ucase(rsEmail.Und_Descricao))#, informamos que está disponível na intranet o Relatório de Controle Interno: Nº #ninsp#, realizada nessa Unidade na Data: #dateformat(rsEmail.INP_DtInicInspecao,"dd/mm/yyyy")#. <br><br><br>
					...
					&nbsp;&nbsp;&nbsp;Registrar a resposta no Sistema Nacional de Controle Interno - SNCI  num prazo de dez (10) dias, contados a partir da data de entrega do Relatório. <br>
					...
					&nbsp;&nbsp;&nbsp;Em caso de dúvidas, entrar em contato com a Equipe de Controle Interno localizada na SE.<br><br>
		...
		<cfoutput query="qNaoRespondido">
			 ...
			 <cfif qNaoRespondido.Pos_Situacao_Resp eq 14>
				 ...
				 <cfquery datasource="#dsn_inspecao#">
				   INSERT Andamento (And_NumInspecao, And_Unidade, And_NumGrupo, And_NumItem, And_DtPosic, And_username, And_Situacao_Resp, And_HrPosic, And_Area)
				   VALUES ('#qNaoRespondido.Pos_Inspecao#', '#qNaoRespondido.Pos_Unidade#', #qNaoRespondido.Pos_NumGrupo#, #Pos_NumItem#, convert(char, getdate(), 102), '#CGI.REMOTE_USER#', 14, '#TimeFormat(Now(),"HH:mm")#:#NumberFormat(qNaoRespondido.CurrentRow,'00')#','#unid#')
				 </cfquery>
    ...
    <cfelse>
	   	<!--- trabalhar com os possiveis intenções de passar do Status = 0-RE ou 11-EL para 19-TA --->
		   <cfset dtnovoprazo = dateformat(now(),"dd/mm/yyyy")>
		   <cfset dtnovoprazo = DateAdd("d", 10, dtnovoprazo)>
			<!--- inicio loop --->
		   <cfset nCont = 1>
		 ...
		 <cfquery datasource="#dsn_inspecao#">
		  ...
		  <cfquery name="rsArea" datasource="#dsn_inspecao#">
		   ...
		   , Pos_Nomearea = '#rsArea.Ars_Sigla#'
		   <cfset Gestor = rsArea.Ars_Sigla>
		   <cfset situacao = 'TRATAMENTO DA AREA'>
	    <cfset Encaminhamento = 'Opinião do Controle Interno'>
		<cfset aux_obs = "Encaminhamos a oportunidade de aprimoramento para as devidas providencias">
	   ...
	   , Pos_Parecer=
		 <cfset pos_aux = #rsPar.Pos_Parecer# & CHR(13) & CHR(13) & DateFormat(Now(),"DD/MM/YYYY") & '-' & TimeFormat(Now(),'HH:MM') & '> ' & #Trim(Encaminhamento)#  & CHR(13) & CHR(13) & 'À Área ' & #Gestor# & CHR(13) & CHR(13) & #aux_obs# & CHR(13) & CHR(13) & 'Data de Previsão da Solução: ' & #DateFormat(dtnovoprazo,"DD/MM/YYYY")# & CHR(13) & CHR(13) & 'Situação: ' & #situacao# & CHR(13) & CHR(13) &  'Responsável: ' & #CGI.REMOTE_USER# & '\' & Trim(qUsuario.Usu_LotacaoNome) & CHR(13) & CHR(13) & '-----------------------------------------------------------------------------------------------------------------------'>
		'#pos_aux#'
	  ...
	  <cfquery name="rsExiste14" datasource="#dsn_inspecao#">
		...
		<cfif rsExiste14.recordcount lt 1>
		  <cfset hhmm_segadic = TimeFormat(Now(),"HH:MM") & ":60">
		  <cfquery datasource="#dsn_inspecao#">
			INSERT Andamento (And_NumInspecao, And_Unidade, And_NumGrupo, And_NumItem, And_DtPosic, And_username, And_Situacao_Resp, And_HrPosic, And_Area)
			VALUES ('#FORM.ninsp#', '#FORM.unid#', #FORM.ngrup#, #FORM.nitem#, convert(char, getdate(), 102), '#CGI.REMOTE_USER#', 14, '#hhmm_segadic#', '#unid#')
		  </cfquery>
	   </cfif>
	  <cfset hhmm_segadic = TimeFormat(Now(),"HH:MM") & ":99">
	  <cfquery datasource="#dsn_inspecao#">
		...
		VALUES ('#FORM.ninsp#', '#FORM.unid#', #FORM.ngrup#, #FORM.nitem#, convert(char, getdate(), 102), '#CGI.REMOTE_USER#', 19, '#hhmm_segadic#', '#and_aux#', '#unid#')
	  </cfquery>
	  <!--- envio de e-mail para area --->

		<cfquery name="rsAreaEmail" datasource="#dsn_inspecao#">
		SELECT Ars_Email FROM Areas WHERE Ars_Codigo='#Form.frmcbarea#'
		</cfquery>
		<cfif rsAreaEmail.recordcount lte 0>
		   <cfset emailarea = 'gilvanm@correios.com.br'>
		<cfelse>
		   <cfset emailarea = trim(rsAreaEmail.Ars_Email)>
		</cfif>
			  <!---  --->
		 <cfif findoneof("@", emailarea eq 0)>
			  <cfset sdestina = "gilvanm@correios.com.br">
		 <cfelse>
			  <cfset sdestina = #emailarea#>
		 </cfif>
		 <cfif left(Form.frmcbarea,2) is '20'>
		   <cfset sdestina = #sdestina# & ';fsobrinho@correios.com.br'>
		 </cfif>
      <!---   <cfset sdestina = "gilvanm@correios.com.br">  --->
		  <cfmail from="SNCI@correios.com.br" to="#sdestina#" subject="Relatório de Avaliação de Controle Interno" type="HTML">
           Mensagem automática. Não precisa responder!<br><br>
		   Ao Gestor do(a) #Ucase(rsArea.Ars_Sigla)#. <br><br><br>
&nbsp;&nbsp;&nbsp;Comunicamos que há pontos  de Controle Interno  "Tratamento da Área" para  manifestação  desse Órgão.<br><br>
...
&nbsp;&nbsp;&nbsp;Para registro de suas manifestações acesse o SNCI clicando no link: <a href="http://intranetsistemaspe/snci/rotinas_inspecao.cfm">Relatório de Controle Interno.</a>Inspeção Nº #FORM.ninsp#, Grupo: #FORM.ngrup#  e Item: #FORM.nitem#.<br><br>
	 </cfmail>
...
<cfif IsDefined("FORM.MM_UpdateRecord") AND FORM.MM_UpdateRecord EQ "form1" And IsDefined("FORM.acao") And Form.acao is "Salvar2">
 ...
 <cfquery datasource="#dsn_inspecao#">
  ...
  <cfset aux_sei = "">
  , Pos_SEI=
 <cfif IsDefined("FORM.frmnumsei") AND len(trim(FORM.frmnumsei)) EQ 20>
...
<cfif rsTPUnid.Und_TipoUnidade is 12>
	...
	<!--- Fazer Contagem do Prazo máximo de 30 dias --->
	   <cfset auxQtdDias = 0>
	   <!--- Status 14-NR --->
	   <cfquery name="rs14NR" datasource="#dsn_inspecao#">
			SELECT And_DtPosic, And_HrPosic, DATEDIFF(d, And_DtPosic, GETDATE()) as qtddias
			FROM Andamento
			WHERE (And_Unidade = '#URL.unid#' and And_NumInspecao = '#URL.ninsp#' AND And_NumGrupo = #URL.ngrup# AND 
			And_NumItem = #URL.nitem# AND And_Situacao_Resp = 14)
			ORDER BY And_DtPosic desc, And_HrPosic desc
	   </cfquery>
	   <cfset auxQtdDias = rs14NR.qtddias>

	   <cfif rs14NR.qtddias gte 30>
		   
		   <!--- Houve resposta 17-RF ou 20-PF ? --->
		   <cfset auxdtinic = rs14NR.And_DtPosic>
		   <cfset subtrai_dias = 0>
		   <cfset soma_dias = 0>
		   <cfset auxPtoAnt = 14>
		   <cfset auxQtdDias = rs14NR.qtddias>
		   <cfquery name="rs17RF20PF" datasource="#dsn_inspecao#">
				SELECT And_Situacao_Resp, And_DtPosic, And_HrPosic, DATEDIFF(d, And_DtPosic, GETDATE()) as qtddias
				FROM Andamento WHERE (And_Unidade = '#URL.unid#' and And_NumInspecao = '#URL.ninsp#' AND And_NumGrupo = #URL.ngrup# AND And_NumItem = #URL.nitem#) AND (And_Situacao_Resp = 17 or And_Situacao_Resp = 20) and (And_DtPosic >= #createodbcdate(CreateDate(Year(rs14NR.And_DtPosic),Month(rs14NR.And_DtPosic),Day(rs14NR.And_DtPosic)))#) and (And_HrPosic <> '#rs14NR.And_HrPosic#') ORDER BY And_DtPosic, And_HrPosic
		   </cfquery> 
		   <cfif rs17RF20PF.recordcount gt 0>
			   <cfloop query="rs17RF20PF">
				   <cfif rs17RF20PF.And_Situacao_Resp is 17>
					  <cfset soma_dias = soma_dias + DateDiff('d', auxdtinic, rs17RF20PF.And_DtPosic)>
					  <cfset auxdtinic = rs17RF20PF.And_DtPosic>
					  <cfset auxPtoAnt = 17>					  
				   <cfelse>	
					  <cfif auxPtoAnt is 17>
						 <cfset auxdtinic = rs17RF20PF.And_DtPosic>					  
					  </cfif>
					  <cfset auxPtoAnt = 20>
				   </cfif>
			   </cfloop>
			   <cfif auxPtoAnt is 20>
				  <!--- A ultima ocorrencia sempre será 20-PF  Contar até o dia atual --->
				  <cfset soma_dias = soma_dias + DateDiff('d', auxdtinic, now())>
			   </cfif>
			   <cfif soma_dias gte 30>
				  <cfset auxQtdDias = soma_dias> 
			   <cfelse>
				  <cfset auxQtdDias = (30 - soma_dias)> 
			   </cfif>
		  </cfif>  
	   <cfelse>
         <!--- Manter o Status e corrigir o campo  Pos_DtPrev_Solucao --->
	     <cfset auxQtdDias = (30 - rs14NR.qtddias)>
       </cfif>
		  <!--- Fim Prazo máximo 30 dias --->
	   <cfif auxQtdDias gte 30>
		   <cfset gil = gil>
			<cfquery name="rsPonto" datasource="#dsn_inspecao#">
				SELECT STO_Codigo, STO_Descricao FROM Situacao_Ponto WHERE STO_Status='A' AND STO_Codigo <> 1 AND STO_Codigo <> 4 AND STO_Codigo <> 6 AND STO_Codigo <> 7 AND STO_Codigo <> 11 AND STO_Codigo <> 14 AND STO_Codigo <> 15  AND STO_Codigo <> 17 AND STO_Codigo <> 18 AND STO_Codigo <> 2 AND STO_Codigo <> 16 AND STO_Codigo <> 22 AND STO_Codigo <> 20
	   ...
	   <cfelse>
			<cfquery name="rsPonto" datasource="#dsn_inspecao#">
					SELECT STO_Codigo, STO_Descricao FROM Situacao_Ponto WHERE STO_Status='A' AND STO_Codigo <> 1 AND STO_Codigo <> 4 AND STO_Codigo <> 6 AND STO_Codigo <> 7 AND STO_Codigo <> 11 AND STO_Codigo <> 14 AND STO_Codigo <> 15  AND STO_Codigo <> 17 AND STO_Codigo <> 18 AND STO_Codigo <> 2 AND STO_Codigo <> 16 AND STO_Codigo <> 22 
					<cfif Trim(rsUsuarioLogado.GrupoAcesso) neq 'GESTORMASTER'>
				   <!---      AND STO_Codigo <> 9 --->
					</cfif>
					 order by STO_Descricao
 ...
 <cfelse>
	<cfquery name="rsPonto" datasource="#dsn_inspecao#">
	 SELECT STO_Codigo, STO_Descricao FROM Situacao_Ponto WHERE STO_Status='A' AND STO_Codigo <> 1 AND STO_Codigo <> 6 AND STO_Codigo <> 7 AND STO_Codigo <> 11 AND STO_Codigo <> 14 AND STO_Codigo <> 18  AND STO_Codigo <> 17 AND STO_Codigo <> 20 AND STO_Codigo <> 22 
	 <cfif Trim(rsUsuarioLogado.GrupoAcesso) neq 'GESTORMASTER'>
	   <!---      AND STO_Codigo <> 9 --->
	 </cfif>
	 order by STO_Descricao	
	 ...
	 if (document.form1.abertura.value == 'Sim')
		{
			...
			{
			alert('Sr. Gestor, o Nº da SE: ' + procse + ' informado no campo Nº Processo difere ao da SE deste Relatório: ' + document.form1.ninsp.value)
			 return false;
			}
...
<table width="85%"  align="center" bordercolor="f7f7f7">
  ...
  <cfoutput>
    ...
    <tr>
      ...
      <td colspan="3" bgcolor="f7f7f7"><span class="exibir">Respons&aacute;vel</span>&nbsp;&nbsp;<cfoutput><strong class="exibir">#qResponsavel.INP_Responsavel#</strong></cfoutput></td>
      </tr>
   <tr class="exibir">
    ...
    <tr class="exibir">
      ...
      <td colspan="4" bgcolor="f7f7f7"><cfoutput><strong class="exibir">#URL.Ninsp#</strong></cfoutput>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;In&iacute;cio Inspe&ccedil;&atilde;o &nbsp;<strong class="exibir"><cfoutput>#DateFormat(qResposta.INP_DtInicInspecao,"dd/mm/yyyy")#</cfoutput></strong></td>
      </tr>
    <tr class="exibir">
...
<td bgcolor="f7f7f7">Valor</td>
	...
	<tr class="exibir">
	  ...
	  <cfif trim(qResposta.RIP_ReincInspecao) eq "">
	...
	<cfoutput>
 	<!--- <cfif (trim(caracvlr) eq "Quantificado")> --->
      ...
      <td colspan="4" bgcolor="f7f7f7">Caracteres:
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Em Risco(R$):
        <input name="frmemrisco" type="text" class="form" onFocus="document.form1.frmemrisco.value=''" onKeyPress="numericos(this.value)" onKeyDown="validacao()" value="#emrisco#" size="22" maxlength="16" onDblClick="exibirMoeda(this.value,<cfoutput>'#emrisco#'</cfoutput>,'frmemrisco','DC')" onChange="exibirMoeda(this.value,<cfoutput>'#emrisco#'</cfoutput>,'frmemrisco','0')">
	  </td>
      </tr>
      <!---
      <cfelse>
		  ...
		  <cfif (qResposta.Pos_Situacao_Resp is 0) or (qResposta.Pos_Situacao_Resp is 11)>			
			<table width="100%" border="0">
			   ...
			   <tr bgcolor="f7f7f7">
				...
				<select name="nci" class="form"  onchange="hanci(); avisonci()">
				...
				<td width="77%" bgcolor="eeeeee" class="exibir"><div id="ncisei">Nº SEI da NCI :&nbsp;&nbsp;
                   <input name="frmnumseinci" id="frmnumseinci" type="text" class="form" OnChange="avisonci()" onKeyPress="numericos();" onKeyDown="validacao(); Mascara_SEI(this);" size="27" maxlength="20" value="<cfoutput>#numncisei#</cfoutput>">
				  </div>
				  </td>
				  </tr>
             <cfelse>
...
<!---     <tr>
 ...
 <!--- <cfset resp = qResposta.Pos_Situacao_Resp> --->
    ...
    <tr>
      <td colspan="5" align="center"><cfoutput>
        <input name="button" type="button" class="botao" onClick="window.open('itens_controle_respostas.cfm?ninsp=#URL.Ninsp#&unid=#URL.Unid#&ngrup=#URL.Ngrup#&nitem=#URL.Nitem#&dtfim=#url.dtfim#&dtinic=#url.dtinic#&SE=#url.SE#&cktipo=#url.cktipo#&selstatus=#selstatus#&StatusSE=#StatusSE#','_self')" value="Voltar">
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
 ...
 <!--- <cfset resp = qResposta.Pos_Situacao_Resp> --->
    <tr bgcolor="f7f7f7">
      ...
      <td bgcolor="eeeeee">
		<cfif (resp neq 24)>
		  <input name="btn_inc_causa" id="btn_inc_causa" type="Submit" class="botao" value="Incluir" onClick="document.form1.acao.value='Incluir_Causa';"></td>
		<cfelse>
		  <input name="btn_inc_causa" id="btn_inc_causa" type="Submit" class="botao" value="Incluir" onClick="document.form1.acao.value='Incluir_Causa';" disabled></td>
		</cfif>
    </tr>
	   ...
	   <tr bgcolor="f7f7f7">
		...
		<table width="100%" border="0">
          <tr bgcolor="f7f7f7">
				...
				<select name="abertura" class="form"  onchange="proc_disc()" onFocus="exibe('N'); travarcombo('kkk','frmResp')" onBlur="exibe(24)">
				...
				<td width="75%" colspan="-3" bgcolor="eeeeee"><div id="dproc">
				 <!--- Se existir dados na sessão, exibir os dados armazenados --->
					<cfset proc = trim(Session.E01.processo)>
					  ...
					  <input name="proc_se" type="text" class="form" value="<cfoutput>#left(ninsp,2)#</cfoutput>" size="3" maxlength="2" readonly>-<input name="proc_num" type="text" class="form" value="<cfoutput>#Mid(proc,3,5)#</cfoutput>" onFocus="exibe('N'); travarcombo('kkk','frmResp')" onBlur="exibe(24)" onKeyPress="numericos()"  size="6" maxlength="5">-<input name="proc_ano" type="text" class="form" onFocus="exibe('N'); travarcombo('kkk','frmResp')" onBlur="exibe(24)" onKeyPress="numericos()" value="<cfoutput>#Right(proc,2)#</cfoutput>" size="2" maxlength="2">
					  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   ...
   <!---   <cfif qVerificaPonto.RecordCount Neq 0> --->
       ...
       <cfelse>
	     ...
	     <!--- <cfset vlrrecaux = trim(Replace(numberformat(qResposta.Pos_VLRecuperado,'999999.00'),'.',','))> --->
       </cfif>
 ...
 <!--- <input name="VLRecuperado" class="form" type="text" size="20" value="<cfoutput>#vlrrecaux#</cfoutput>" onKeyPress="moedas(VLRecuperado.value)" onKeyDown="validacao()"></div> --->
	  </td>
        </tr>
        <tr bgcolor="f7f7f7">
	  ...
	  <tr>
		...
		<table width="100%" border="0">
		  <cfif trim(qResposta.Pos_Processo) eq ''>
		   <tr bgcolor="f7f7f7">
            <td colspan="5" valign="middle" bgcolor="eeeeee" class="exibir">Foi aberto processo disciplinar?&nbsp;&nbsp;
            <select name="abertura" class="form"  onchange="proc_disc()" onFocus="exibe('N'); travarcombo('kkk','frmResp')" onBlur="exibe(24)">
			<option value="Não" selected>Não</option>
			<option value="Sim">Sim</option>
            </select>
			...
			<cfset aux_ano = right(dateformat(now(),"YYYY"),2)>
            <td width="77%" bgcolor="eeeeee"><div id="dproc">
              &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="exibir">Nº Processo:</span>&nbsp;&nbsp;
  ...
  <!---    <tr>
      ...
      <tr bgcolor="f7f7f7">
        <td height="22" colspan="2" valign="middle" bgcolor="eeeeee" class="exibir">Situação:
              <select name="frmResp" class="form" id="frmResp" onChange="exibe(this.value)">
			  <option selected="selected" value="N">---</option>
              <cfoutput query="rsPonto">
                <cfif STO_Codigo neq 0 and STO_Codigo neq 1>
...
<!--- 	 <cfset vlrrecaux = trim(Replace(numberformat(qResposta.Pos_VLRecuperado,'999999.00'),'.',','))> --->
	...
	<!---  <cfif qVerificaPonto.RecordCount Neq 0>  --->
		   <td colspan="2" bgcolor="eeeeee" class="exibir"><div id="dValor">Valor Regularizado(R$):&nbsp;&nbsp;
		   <input name="VLRecuperado" type="text" class="form" onKeyPress="moedas(this.value,'VLRecuperado')" onKeyDown="validacao()" value="<cfoutput>#vlrrecaux#</cfoutput>" size="22" maxlength="16" onDblClick="exibirMoeda(this.value,<cfoutput>'#vlrrecaux#'</cfoutput>,'VLRecuperado','DC')" onChange="exibirMoeda(this.value,<cfoutput>'#vlrrecaux#'</cfoutput>,'VLRecuperado','0')">
			 <!--- <input name="VLRecuperado" value="<cfoutput>#vlrrecaux#</cfoutput>" class="form" type="text" size="20" onKeyPress="moedas(VLRecuperado.value)" onKeyDown="validacao()"></div> --->
		  </td>
<!--- 	  <cfelse>
			  <td colspan="3" bgcolor="eeeeee" class="exibir"><div id="dValor">Valor Recuperado(R$):&nbsp;&nbsp;
 ...
 <td colspan="5" align="center"><div align="center">
   ...
   <input name="button" type="button" class="botao" onClick="window.open('itens_controle_respostas.cfm?ninsp=#URL.Ninsp#&unid=#URL.Unid#&ngrup=#URL.Ngrup#&nitem=#URL.Nitem#&dtfim=#url.dtfim#&dtinic=#url.dtinic#&SE=#url.SE#&cktipo=#url.cktipo#&selstatus=#selstatus#&StatusSE=#StatusSE#','_self')" value="Voltar">
	   ...
	   <cfset numdias = qResposta.diasOcor>
	  	<cfif ((resp is 1 and numdias lte 2) or (resp is 6 and numdias lte 2) or (resp is 7 and numdias lte 2) or (resp is 17 and numdias lte 2) or (resp is 18) or (resp is 20) or (resp is 24))>
